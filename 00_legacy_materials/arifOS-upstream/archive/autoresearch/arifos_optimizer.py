...
class ArifOSOptimizer:
    ...
    def suggest_next_experiment(self) -> Tuple[str, Dict]:
        ...
        max(kept_experiments, key=lambda x: x["composite_score"])
    ...
    def print_status(self):
        ...
        if history:
            ...
            for exp in history[-5:]:
                ...
                print(
                    f"  {status} {exp['experiment_id']}: {exp['composite_score']:.4f} — {exp['change_description'][:40]}"
                )
        else:
...
def main():
    ...
    if args.overnight:
        # Overnight mode
        optimizer.run_overnight(max_experiments=20, experiment_duration=args.duration)
        return 0
