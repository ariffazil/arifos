...
# Test A2A Agent Card
curl https://mcp.arif-fazil.com/.well-known/agent.json | jq

# Test A2A task submission
curl -X POST https://mcp.arif-fazil.com/a2a/task \
  -H "Content-Type: application/json" \
  -d '{
...
curl https://mcp.arif-fazil.com/.well-known/webmcp

# Test WebMCP console (should return HTML)
curl https://mcp.arif-fazil.com/webmcp | head -20
```
...
|------|---------|----------|
| **MCP Tools** | `curl -X POST /mcp -d '{"method":"tools/list"}'` | List of 13 canonical tools |
...
3. **Test WebMCP**: Open browser to `https://mcp.arif-fazil.com/webmcp`
