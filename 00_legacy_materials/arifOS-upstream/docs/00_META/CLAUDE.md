...
# Run MCP Server — canonical entry points
python -m arifosmcp.mcp_server           # stdio (Claude Desktop, local agents)
python -m arifosmcp.runtime stdio        # stdio compatibility shim
uvicorn arifosmcp.runtime.server:app --host 0.0.0.0 --port 8080   # Streamable HTTP at /mcp
...
| ----------------- | ------------------------------------------------------------ | ---------------------------- |
| Local (stdio)     | `python -m arifosmcp.mcp_server`                                | Claude Desktop, Cursor IDE   |
| Local (compat)    | `python -m arifosmcp.runtime stdio`                             | Legacy-compatible stdio shim |
| VPS/HTTP          | `uvicorn arifosmcp.runtime.server:app --host 0.0.0.0 --port 8080` | Public streamable-http runtime |
...
- Status: `https://mcp.arif-fazil.com/api/status`

> ⚠️ **LEGACY ALIAS**: `arifosmcp.arif-fazil.com` is not the canonical public host. Always use `mcp.arif-fazil.com` for the live public MCP surface (**13 tools / 11 prompts / 3 resources**).
>
