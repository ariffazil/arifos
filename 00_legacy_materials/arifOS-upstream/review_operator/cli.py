...
# ============================================================
DB_DSN = os.getenv(
    "VAULT999_DB", "postgresql://vault_writer_svc:VaultWriterSecret2026!@72.62.71.199:5432/vault999"
)
...
async def cmd_recent_reviews(conn, limit: int = 10):
    ...
    for r in rows:
        print(
            f"  [{r['decision']}] {r['reviewed_at']} | {r['human_signature']} | {r['reason'][:60]}..."
        )
...
async def main():
    ...
    try:
        if args.cmd == "list":
            ...
            await cmd_ratify(
                conn, args.cooling_id, args.decision.upper(), args.reason, args.irr_ack
            )
