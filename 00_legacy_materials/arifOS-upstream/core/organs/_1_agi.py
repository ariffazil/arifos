...
async def agi(
    ...
    from core.organs.anthropic_agent import anthropic_generate
    from core.organs.sea_lion_agent import sea_lion_generate
    from core.organs.minimax_agent import minimax_generate

    async def llm_call(prompt: str, max_tokens: int):
        # Try Anthropic first (verified working)
        envelope = await anthropic_generate(prompt=prompt, max_tokens=max_tokens)
        if not envelope.ok:
            logger.warning(f"Anthropic failed: {envelope.error}. Trying SEA-LION...")
            # Fallback to SEA-LION
            envelope = await sea_lion_generate(prompt=prompt, max_tokens=max_tokens)
        if not envelope.ok:
