...
def run_mcp_test():
    ...
    resp = send(
        "tools/call",
        {"name": "arifos_mind", "arguments": {"mode": "reason", "session_id": "test"}},
        2,
    )
    if resp and "result" in resp:
    ...
    resp = send(
        ...
        {
            "name": "arifos_memory",
            "arguments": {"mode": "vector_store", "content": "", "session_id": "test"},
        },
        3,
    )
    if resp and "result" in resp:
    ...
    resp = send(
        "tools/call",
        {"name": "arifos_ops", "arguments": {"mode": "vitals", "session_id": "test"}},
        4,
    )
    if resp and "result" in resp:
        content = json.loads(resp["result"]["content"][0]["text"])
