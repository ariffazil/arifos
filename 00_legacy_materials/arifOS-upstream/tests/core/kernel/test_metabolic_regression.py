...
async def test_arifos_kernel_regression_sealed_verdict():
    ...
    init_env = await arifos_init(
        mode="init", declared_name="regression-test-bot", session_id=session_id
    )
    auth_context = (
        init_env.get("auth_context")
        if isinstance(init_env, dict)
        else getattr(init_env, "auth_context", None)
    )
