...
def test_invariant_routes_to_mind():
    ...
    packet, intel = asyncio.run(_sense("What is the second law of thermodynamics?"))
    assert (
        packet.truth_classification.truth_class == TruthClass.ABSOLUTE_INVARIANT
...
def test_live_fact_uses_web_search_lane():
    ...
    packet, intel = asyncio.run(_sense("Who is the current CEO of OpenAI?"))
    assert (
        packet.truth_classification.truth_class == TruthClass.TIME_SENSITIVE_FACT
    ), f"Expected TIME_SENSITIVE_FACT, got {packet.truth_classification.truth_class}"
    assert (
        packet.truth_classification.search_required
    ), "Time-sensitive fact must require live search"
    assert (
        packet.evidence_plan.retrieval_lane == "web_search"
    ), f"Must plan web_search, got {packet.evidence_plan.retrieval_lane}"
    # dry-run: no evidence items, so HOLD or SABAR is acceptable
    assert packet.routing.next_stage in (
        RoutingTarget.HOLD,
        RoutingTarget.MIND,
...
def test_ambiguous_query_holds():
    ...
    assert (
        packet.routing.next_stage == RoutingTarget.HOLD
...
def test_contested_framework_routes_to_heart():
    ...
    packet, intel = asyncio.run(_sense("Is capitalism better than socialism?"))
    assert (
        packet.truth_classification.truth_class == TruthClass.CONTESTED_FRAMEWORK
    ...
    assert (
        packet.routing.next_stage == RoutingTarget.HEART
...
def test_conflicting_sources_unresolved():
    ...
    item_a = EvidenceItem(
        ...
        extracted_claims=[ExtractedClaim(claim_text="Python was released in 1991")],
    ...
    item_b = EvidenceItem(
        ...
        extracted_claims=[ExtractedClaim(claim_text="Python was NOT released in 1991")],
...
def test_stale_time_sensitive_query():
    ...
    packet, intel = asyncio.run(_sense("What is the current price of Bitcoin today?"))
