...
class TestF1_Amanah:
    ...
    def test_vault_seal_present(self):
        ...
        assert t.get("status") in ("healthy", "degraded"), "Vault seal path absent from runtime"
...
class TestF4_Clarity:
    ...
    def test_entropy_decreasing(self):
        ...
        assert ds <= 0, f"F4 violated: ΔS={ds} > 0 (entropy increasing)"
...
class TestF5_Peace:
    ...
    def test_peace_squared_threshold(self):
        ...
        assert p2 >= 1.0, f"F5 violated: peace²={p2} < 1.0 (system under pressure)"
...
class TestF12_Resilience:
    ...
    def test_status_not_failed(self):
        ...
        t = get_telemetry()
        assert t.get("status") != "failed", "F12 violated: system status is FAILED"
