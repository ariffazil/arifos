...
async def test_phase0_hardening():
    ...
    try:
        ...
        result = await client.call_tool(
            "arifos_mind", {"mode": "reason", "session_id": "test-session"}
        )
        content = json.loads(result["result"]["content"][0]["text"])
        if content.get("ok") is False:
            print(
                f"   ✅ Missing query handled: {content.get('verdict')} - {content.get('errors', [{}])[0].get('code', 'UNKNOWN')}"
            )
        else:
        ...
        result = await client.call_tool(
            "arifos_mind", {"mode": "invalid_mode", "query": "test", "session_id": "test-session"}
        )
        content = json.loads(result["result"]["content"][0]["text"])
        ...
        result = await client.call_tool(
            "arifos_memory", {"mode": "vector_store", "content": "", "session_id": "test-session"}
        )
        content = json.loads(result["result"]["content"][0]["text"])
        if content.get("ok") is False:
        ...
        result = await client.call_tool(
            "arifos_memory", {"mode": "vector_forget", "session_id": "test-session"}
        )
        content = json.loads(result["result"]["content"][0]["text"])
        if content.get("ok") is False:
            print(
                f"   ✅ Missing identifiers handled: {content.get('errors', [{}])[0].get('code')}"
            )
        else:
            print(f"   ⚠️ Response: {content.get('ok', 'unknown')}")
        ...
        result = await client.call_tool(
            "arifos_ops", {"mode": "vitals", "action": "system_check", "session_id": "test-session"}
        )
        content = json.loads(result["result"]["content"][0]["text"])
        ...
        result = await client.call_tool(
            "arifos_ops", {"mode": "health", "session_id": "test-session"}
        )
        content = json.loads(result["result"]["content"][0]["text"])
        if content.get("ok") is True:
