...
async def test_successful_seal_path(conn_vw):
    ...
    try:
        ...
        seal = await conn.fetchrow(
            "SELECT id, verdict, cooling_id FROM vault_seals WHERE id = $1", seal_id
        )
        ...
        log(
            cq and cq["status"] == "awaiting_human",
            "cooling_queue status unchanged (awaiting update)",
        )
...
async def test_append_only_blocks_update(conn_vw):
    ...
    try:
        ...
        try:
            await conn_vw.execute(
                "UPDATE vault_seals SET metadata = metadata WHERE id = $1", seal_id
            )
...
async def test_append_only_blocks_delete(conn_vw):
    ...
    conn = await get_conn()
    try:
        seal_id = await conn.fetchval(
            "SELECT id FROM vault_seals ORDER BY ratified_at DESC LIMIT 1"
        )
