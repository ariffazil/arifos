...
class ODSiphon:
    ...
    def export_causal_scene(self, horizon_id=None, well_name=None, seis_id=None):
        ...
        if horizon_id:
            scene["witnesses"].append(self.distill_claim(horizon_id))
        if well_name:
            scene["witnesses"].append(self.distill_truth(well_name))
        if seis_id:
            scene["witnesses"].append(self.distill_texture(seis_id))
