...
class DepthSample(BaseModel):
    ...
    resistivity: float = Field(
        ..., ge=0.01, le=10000, description="Electrical resistivity ρₑ [Ω·m]"
    )
    magnetic_suscept: float = Field(
        default=0.0, ge=-1, le=100, description="Magnetic susceptibility χ [SI]"
    )
    thermal_conduct: float = Field(
        default=2.5, ge=0.5, le=10, description="Thermal conductivity k [W/(m·K)]"
    )
    ...
    def to_array(self) -> np.ndarray:
        ...
        return np.array(
            [
                ...
                self.vs,
                self.resistivity,
                self.magnetic_suscept,
                self.thermal_conduct,
                self.pressure,
                self.temperature,
                self.porosity,
            ]
    ...
    def from_array(cls, depth: float, arr: np.ndarray, **kwargs) -> DepthSample:
        ...
        return cls(
            ...
            vs=arr[2],
            resistivity=arr[3],
            magnetic_suscept=arr[4],
            thermal_conduct=arr[5],
            pressure=arr[6],
            temperature=arr[7],
            porosity=arr[8],
            **kwargs,
