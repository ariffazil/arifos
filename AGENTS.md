...
| `aaa_mcp/` | TRANSPORT ADAPTER | MCP protocol, session management, metabolic pipeline, NO decision logic |
...
|----------|----------|-------------|
| Unit Tests | `tests/core/`, `tests/aclip_cai/` | Component-level tests |
...
### Local Development
```bash
python -m arifos_aaa_mcp stdio    # For Claude Desktop, Cursor
```

### VPS/Coolify (Production)
```bash
python -m arifos_aaa_mcp          # SSE mode, port 8080
```
...
| `aclip_cai/` has NO HTTP servers | Perception only, no transport |
