...
def patch_file(filepath):
    ...
    content = re.sub(r'def _arif_ping\(\n.*?_envelope: dict\[str, Any\] \| None = None,\n\)\s*->', 
                     lambda m: m.group(0).replace('_envelope: dict[str, Any] | None = None,', f'{sig}'), content, flags=re.DOTALL)

    # Patch arif_schema_echo
    content = re.sub(r'def arif_schema_echo\(params: dict = \{\}\)\s*->', 
                     f'def arif_schema_echo(\n    {sig}\n) ->', content)
    # Fix the usage of params inside the first arif_schema_echo
    content = content.replace("received_keys = sorted(params.keys())", "params = payload if isinstance(payload, dict) else {}\n    received_keys = sorted(params.keys())")

    # Patch arif_transport_echo
    content = re.sub(r'def arif_transport_echo\(\n    protocol_version: str = "unknown",\n    client_name: str = "unknown",\n    client_version: str = "unknown",\n    transport: str = "streamable_http",\n\)\s*->',
    ...
    content = content.replace('_arif_schema_echo(payload=payload, _envelope=_envelope)', '_arif_schema_echo(payload=payload, _envelope=_envelope, client_capabilities=client_capabilities)')
    
    # _arif_version_echo(_envelope=_envelope)
    content = content.replace('_arif_version_echo(_envelope=_envelope)', '_arif_version_echo(payload=payload, _envelope=_envelope, client_capabilities=client_capabilities)')
    
    # _arif_transport_echo(_envelope=_envelope)
    content = content.replace('_arif_transport_echo(_envelope=_envelope)', '_arif_transport_echo(payload=payload, _envelope=_envelope, client_capabilities=client_capabilities)')
    
    # _arif_initialize_probe(protocol_version=protocol_version, client_capabilities=client_capabilities, _envelope=_envelope)
    content = content.replace('_arif_initialize_probe(\n            protocol_version=protocol_version,\n            client_capabilities=client_capabilities,\n            _envelope=_envelope,\n        )', 
