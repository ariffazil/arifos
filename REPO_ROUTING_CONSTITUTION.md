...
|------|---------------|---------------|--------|
| **AAA** | Agent workspace, governance ADRs, orchestration canon, AAA–arifOS–A-FORGE routing policy | `/root/AAA/` | `github.com/ariffazil/AAA` |
...
**OpenClaw workspace:** `/srv/openclaw/workspace/` — this is the agent's operative home.
**Hermes workspace:** `/root/.hermes/workspace/` — ASI agent, reads from its own workspace.
...
|---------|----------------|-------|
| OpenClaw (main) | `/srv/openclaw/workspace/` + any repo's working dir | Full read/write within repos |
| Hermes (ASI) | `/root/.hermes/workspace/` + repos via explicit path | Reads constitution from its workspace |
...
| A-FORGE planning twin update | `/root/A-FORGE/` | `aforge/planning-twim-v3` | Design/architecture |
