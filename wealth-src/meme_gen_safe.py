...
def fit_text(draw, text, box_w, box_h, max_size=64, min_size=14, stroke=4):
    ...
    for size in range(max_size, min_size - 1, -1):
        ...
        bbox = draw.multiline_textbbox(
            (0, 0), wrapped, font=font, align="center", spacing=4, stroke_width=stroke
        )
        w = bbox[2] - bbox[0]
        h = bbox[3] - bbox[1]
...
def draw_meme(canvas, top_text, bottom_text, accent=(255, 210, 0), theme="default"):
    ...
    if theme == "nanny_vs_chaos":
        ...
        draw.line(
            [(sh[0] + 10, sh[1] + 20), (sh[2] - 10, sh[3] - 10)], fill=(240, 240, 240), width=3
        )
        ...
        # ears
        draw.ellipse(
            (rb[0] - 30, cy - 30, rb[0] + 30, cy + 30),
            fill=(140, 90, 55),
            outline=(240, 240, 240),
            width=3,
        )
        draw.ellipse(
            (rb[2] - 30, cy - 30, rb[2] + 30, cy + 30),
            fill=(140, 90, 55),
            outline=(240, 240, 240),
            width=3,
        )
        ...
        draw.polygon(
            [(W // 2 - 10, cy - 70), (W // 2 + 10, cy - 70), (W // 2, cy - 35)], fill=accent
        )
        draw.polygon(
            [(W // 2 - 10, cy + 70), (W // 2 + 10, cy + 70), (W // 2, cy + 35)], fill=accent
        )
        ...
        # stamp
        draw.ellipse(
            (cb[2] - 90, cb[1] + 140, cb[2] - 20, cb[1] + 210),
            fill=(220, 60, 60),
            outline=(255, 255, 255),
            width=3,
        )
        draw.text(
            (cb[2] - 55, cb[1] + 175), "SAFE", fill=(255, 255, 255), anchor="mm", font=load_font(28)
        )
        ...
        draw.rounded_rectangle(
            (mid_box[0] + 50, y, mid_box[2] - 50, y + 40), radius=20, fill=(80, 100, 140)
        )
    ...
    # Fit top
    font_top, wrapped_top = fit_text(
        draw,
        top_text.upper(),
        top_box[2] - top_box[0] - 2 * pad,
        top_box[3] - top_box[1] - 2 * pad,
        max_size=56,
        min_size=18,
        stroke=stroke,
    )
    bbox_top = draw.multiline_textbbox(
        (0, 0), wrapped_top, font=font_top, align="center", spacing=4, stroke_width=stroke
    )
    ...
    # Fit bottom
    font_bot, wrapped_bot = fit_text(
        draw,
        bottom_text.upper(),
        bottom_box[2] - bottom_box[0] - 2 * pad,
        bottom_box[3] - bottom_box[1] - 2 * pad,
        max_size=48,
        min_size=16,
        stroke=stroke,
    )
    bbox_bot = draw.multiline_textbbox(
        (0, 0), wrapped_bot, font=font_bot, align="center", spacing=4, stroke_width=stroke
    )
...
img1 = make_canvas()
draw_meme(
    img1, "Nanny Bot vs Chaos Monkey", "Who wins? Production still loses.", theme="nanny_vs_chaos"
)
...
draw_meme(
    img2,
    "AI safety overlords",
    "Just one more polic🪢y and your model will be saf🪢e",
    theme="overlords",
)
...
img3 = make_canvas()
draw_meme(
    img3, "When they say: alignment first", "But the bug was in the data pipeline.", theme="default"
)
