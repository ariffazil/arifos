...
def create_perception_mcp() -> FastMCP:
    ...
    def P_geox_scene_load(
        ...
        base = Path("/root/GEOX")
        search_paths = [Path(path), base / path, base / "data" / path, base / "prospects" / path]
        found_path = None
        ...
        if found_path:
            ...
            status = "loaded"
            scene_id = f"{scene_type}_{Path(found_path).name}"
        else:
            size = 0
            status = "not_found"
            scene_id = f"{scene_type}_unknown"
    ...
    def P_geox_skills_query(
        ...
        registry_path = Path("/root/GEOX/registry/registry.json")
        if not registry_path.exists():
            return {
                "agent": "P",
                "source": "GEOX",
                "action": "skills_query",
                "data": {"query": query, "domain": domain, "skills": [], "error": "registry_not_found"},
            }
        ...
        if isinstance(skills, dict):
            skill_list = []
            for domain_skills in skills.values():
                if isinstance(domain_skills, list):
                    skill_list.extend(domain_skills)
                else:
                    skill_list.append(domain_skills)
        else:
        ...
        filtered = [
            ...
            if q in (s.get("name", "") + " " + s.get("description", "") + " " + s.get("domain", "")).lower()
        ]
