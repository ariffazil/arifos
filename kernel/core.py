        ...
        **kwargs: Any,
        ...
        if session_id and session_id != "global":
            ...
            if _identity:
                ...
                is_polluted = _identity.get("is_polluted", False)
                authority_level = _identity.get("authority_level", "anchored")
        
        effective_actor = (
            _bound_actor or payload.get("actor_id") or actor_id or "anonymous"
        ).strip().lower()

        # ── Kernel Router Schema Validation (Next Horizon) ──
        mode_val = payload.get("mode", KernelTaskMode.METABOLIC)
        try:
            router_input = KernelRouterInput(
            ...
            logger.warning(f"Kernel Input Validation Failed: {e}")
            router_input = None
        ...
        context = {
            ...
            "is_polluted": is_polluted or payload.get("is_polluted", False),
            "risk_tier": risk_tier,
            "dry_run": dry_run,
    ...
    async def _invoke_with_governance(
        ...
        try:
            kwargs = {
                ...
                "is_polluted": context.get("is_polluted", False),
...
async def arifOS_kernel(**kwargs: Any) -> dict[str, Any]:
    """Canonical arifOS Decision Kernel."""
    return await get_kernel_core().execute(**kwargs)


async def kernel_execute(**kwargs: Any) -> dict[str, Any]:
    """Compatibility shim for legacy arifos_kernel."""
    return await arifOS_kernel(**kwargs)
