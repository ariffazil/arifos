...
def arifos_guarded_tool(reversible: bool = False, risk_tier: str = "high"):
    """
    ...
    def decorator(func: Callable[..., Awaitable[Any]]) -> Callable[..., Awaitable[Any]]:
        ...
        async def wrapper(*args, **kwargs) -> Any:
            ...
            if verdict == "SEAL":
                ...
                raise ArifOSRejectionError(
                    f"arifOS requested SABAR (patience/replan): {judge_output.advisory}"
                )
            elif verdict == "HOLD":
                raise ArifOSRejectionError(
                    f"arifOS issued HOLD (human review required): {judge_output.advisory}"
                )
            else:
                raise ArifOSRejectionError(
                    f"arifOS issued VOID (action forbidden): {judge_output.advisory}"
                )
