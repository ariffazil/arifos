"""
arifos.governance - Safety & Audit Module

Contains governance and audit components:
- fag: File Access Guardian
- ledger: Cooling ledger operations (MOVED to arifos.state in v47)
- ledger_hashing: Hash chain integrity (MOVED to arifos.state in v47)
- merkle: Merkle proofs (MOVED to arifos.state in v47)
...
def __getattr__(name):
    ...
    if name in state_modules:
        warnings.warn(
            f"Importing {name} from arifos.apex.governance is deprecated. "
            f"Use 'from arifos.state import {name}' instead. "
