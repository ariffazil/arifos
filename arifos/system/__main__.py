...
Usage (PowerShell examples):
  python -m arifos.system --query "test query" --verbose

Legacy compatibility: python -m arifos.system.pipeline --query "test"
"""
...
from arifos.system.runtime.bootstrap import ensure_bootstrap, get_bootstrap_payload

# AAA-Level: Import quantum helpers instead of old pipeline
from arifos.mcp import validate_text_sync

logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] %(name)s: %(message)s",
)
logger = logging.getLogger("arifos.system.__main__")


from arifos.memory.ledger.cooling_ledger import DEFAULT_LEDGER_PATH, append_entry
