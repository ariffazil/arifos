...
# NOTE: These imports require spec files. During single-body consolidation (v49),
# they're optional to allow metabolizer/orchestrator to work independently.
try:
    from .enforcement.metrics import ConstitutionalMetrics, FloorsVerdict, Metrics
except (ImportError, RuntimeError):
    # Spec files not available - stub types for development
    ConstitutionalMetrics = None
    FloorsVerdict = None
    Metrics = None
...
try:
    ...
    # Spec files not available - stub for development
    APEX_EPOCH = None
    APEX_VERSION = None
    APEXPrime = None
    ApexVerdict = None
...
try:
    ...
    def log_cooling_entry(*args, **kwargs):
        ...
        import logging
        logging.getLogger("arifos").warning(
            "log_cooling_entry unavailable - using stub. Spec files not loaded."
        )
        return {
            "status": "stub",
...
try:
    ...
    AGI = None
    ASI = None
    RED_PATTERN_SEVERITY = None
    RED_PATTERN_TO_FLOOR = None
    RED_PATTERNS = None
    Accountant = None
    AccountantResult = None
    ASIResult = None
    EvaluationMode = None
    EvaluationResult = None
    Sentinel = None
    SentinelResult = None
    evaluate_session = None
