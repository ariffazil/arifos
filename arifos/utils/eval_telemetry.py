...
This module provides an OPTIONAL bridge from arifos to arifos_eval.
It does NOT change existing verdict logic in APEX_PRIME.py.
It only logs telemetry for comparison and validation.

Usage:
    from arifos.enforcement.eval_telemetry import log_eval_telemetry, EVAL_TELEMETRY_ENABLED
...
def _get_eval_instance() -> Optional[Any]:
    ...
    This ensures arifos never hard-depends on arifos_eval.
    """
...
def compare_verdicts(
    ...
    Args:
        core_verdict: Verdict from arifos APEX_PRIME
        dials: A/P/E/X dial values
