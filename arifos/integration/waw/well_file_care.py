...
class WellFileCare:
    ...
    def check_health(self) -> WellHealthReport:
        ...
        # 2. Check arifos structure (if exists)
        arifos = self.repo_root / "arifos"
        if arifos.exists():
            # Count top-level files (should be minimal after reorganization)
            top_level_files = [
                f for f in arifos.glob("*.py")
                if f.name != "__init__.py"
            ]
            if len(top_level_files) > 10:
                report.warnings.append(
                    f"arifos has {len(top_level_files)} top-level files "
...
def create_well_file_care(repo_root: Optional[str] = None) -> WellFileCare:
    ...
    Usage:
        from arifos.integration.waw.well_file_care import create_well_file_care
