...
> **arifOS makes it structurally incorruptible — not by discipline, but by protocol.**
...
**GENESIS/047 solves the one problem Beer never solved: S2.**
...
The arifOS S2 Coordination Protocol solves this with three mechanical invariants:
...
These are not policies. They are protocol constraints — enforced at transport level, not agent level.
...
**Implementation:**
- Every organ publishes a `coordination_heartbeat` to NATS JetStream subject `arifos.coordination.{organ_id}` on every state change and on a TTL heartbeat (default: 30s)
...
**This closes the "coordination is implicit" gap.** The forge pipeline no longer assumes coherence — it verifies it, mechanically, before every execution.

### 3.3 Invariant C3 — Drift-Is-Event-Not-Probe

> *Coordination failures must surface as NATS events on `arifos.coordination.drift` within 500ms of detection. Health probes are supplementary, not primary.*
...
**NATS JetStream** — already deployed in arifOS infrastructure (GENESIS/046 §5.3 notes it is live).
...
S2 protocol is structurally incorruptible because the verification is **upstream of the decision**. By the time the agent reaches the decision point, the coordination check has already run. There is no human-in-the-loop moment where "skip coordination" is an option. The option does not exist in the protocol.
...
| GENESIS/046 — Constitutional VSM | §5 diagnosed S2 weakness; this document is the solution |
...
*Forged 2026-06-29 — the day S2 stopped being a weakness and became a wire.*
