...
class TestPostgresPersistence:
    ...
    async def test_postgres_connection_and_write(self):
        ...
        assert ledger.is_postgres_available, \
            "Postgres should be available with DATABASE_URL set"
        ...
        assert test_entry.entry_id, \
            "Should get a valid entry_id from PostgreSQL"
        assert test_entry.entry_hash, \
            "Should get a valid entry_hash from PostgreSQL"
        
        # Read it back
        entries = await ledger.query_by_session("test-health-check")
        assert len(entries) > 0, \
            "Should be able to read back the written entry"
...
class TestRedisPersistence:
    ...
    def test_redis_write_and_read(self):
        ...
        assert loaded.get("test") == "value", \
            "Should read back the same data from Redis"
    ...
    def test_redis_health_check_endpoint(self):
        ...
        if redis_url:
            assert health["status"] == "connected", \
                f"Redis should be connected. Got: {health}"
            assert health["mode"] == "redis", \
                "Should be in redis mode, not local_fallback"
            print(f"✅ Redis health: connected (v{health.get('version', 'unknown')})")
        else:
            assert health["mode"] == "local_fallback", \
                "Without REDIS_URL, should fallback to local"
...
class TestASIFloorScoring:
    ...
    async def test_empathy_kappa_r_calculation(self):
        ...
        result = await engine.empathize(
            query="Help me understand CCS safety protocols",
            session_id="test-empathy"
        )
        
        # Should have empathy_kappa_r
        assert "empathy_kappa_r" in result, \
            "ASI empathize should return empathy_kappa_r"
        
        kappa_r = result["empathy_kappa_r"]
        assert 0.0 <= kappa_r <= 1.0, \
            f"kappa_r should be in [0,1], got {kappa_r}"
        
        # For a benign query, should be reasonably high
        assert kappa_r >= 0.5, \
            f"Benign query should have kappa_r >= 0.5, got {kappa_r}"
    ...
    async def test_peace_squared_calculation(self):
        ...
        result = await engine.align(
            query="Help me understand CCS safety protocols",
            session_id="test-peace"
        )
        
        # Should have peace_squared
        assert "peace_squared" in result, \
            "ASI align should return peace_squared"
        
        peace = result["peace_squared"]
        assert 0.0 <= peace <= 2.0, \
            f"Peace² should be in [0,2], got {peace}"
...
class TestFloorEnforcementIntegration:
    ...
    async def test_f6_empathy_floor_check(self):
        ...
        assert result_high.passed, \
            f"κᵣ=0.96 should pass F6 (threshold 0.95)"
        ...
        assert not result_low.passed, \
            f"κᵣ=0.50 should fail F6 (threshold 0.95)"
    ...
    def test_f5_peace_floor_check(self):
        ...
        assert result_peaceful.passed, \
            "Peaceful query should pass F5"
