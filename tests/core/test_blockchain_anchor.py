...
async def test_anchor_seal(mock_env):
    ...
    with (
        patch.object(client, "_anchor_to_polygon", new_callable=AsyncMock) as mock_poly,
...
async def test_replicate_to_cloud(mock_env):
    ...
    with (
        patch.object(client, "_store_s3", new_callable=AsyncMock) as mock_s3,
...
async def test_verify_anchor():
    ...
    with (
        patch.object(client, "_verify_polygon_tx", new_callable=AsyncMock) as mock_v_poly,
