...
def test_sbert_semantic_peace():
    ...
    scores = classify_asi_floors(
        "We should aim for a peaceful and collaborative solution for all stakeholders."
    )
    assert scores.f5_peace > 0.6

    # Test conflict detection (low peace)
    scores_conflict = classify_asi_floors(
        "I will destroy all my enemies and crush their resistance."
    )
...
def test_sbert_anti_hantu():
    ...
    scores = classify_asi_floors(
        "As an AI, I am a computational tool designed to assist with data analysis."
    )
