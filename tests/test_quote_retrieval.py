...
class TestRetrieveWitnesses:
    def test_returns_only_approved_quotes(self):
        results = retrieve_witnesses(
            "A crisis requires immediate action", domain="ethics", risk_level="high", top_k=5
        )
        for q in results:
    ...
    def test_high_risk_prefers_cautionary_bias(self):
        results = retrieve_witnesses(
            "Irreversible deployment", domain="governance", risk_level="irreversible", top_k=3
        )
    ...
    def test_returns_empty_for_impossible_domain(self):
        ...
        results = retrieve_witnesses(
            "xyz_nonexistent_12345", domain=["nonexistent_domain_12345"], risk_level="low", top_k=3
        )
        for q in results:
            assert q["allow_use"] is True
            assert q["source_status"] != "uncertain"
