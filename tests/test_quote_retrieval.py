...
class TestRetrieveWitnesses:
    def test_returns_only_approved_quotes(self):
        results = retrieve_witnesses("A crisis requires immediate action", domain="ethics", risk_level="high", top_k=5)
        for q in results:
            assert q["allow_use"] is True
            assert q["source_status"] != "uncertain"

    def test_returns_at_most_top_k(self):
        results = retrieve_witnesses("Test event", domain="governance", risk_level="medium", top_k=3)
        assert len(results) <= 3

    def test_ranks_domain_matches_higher(self):
        results = retrieve_witnesses("Governance crisis", domain="governance", risk_level="high", top_k=5)
        if len(results) >= 2:
    ...
    def test_high_risk_prefers_cautionary_bias(self):
        results = retrieve_witnesses("Irreversible deployment", domain="governance", risk_level="irreversible", top_k=3)
    ...
    def test_returns_empty_for_impossible_domain(self):
        ...
        results = retrieve_witnesses("xyz_nonexistent_12345", domain=["nonexistent_domain_12345"], risk_level="low", top_k=3)
        for q in results:
            assert q["allow_use"] is True
            assert q["source_status"] != "uncertain"
    ...
    def test_result_contains_all_schema_fields(self):
        ...
        if results:
            q = results[0]
            required = {"id", "quote", "author", "tradition", "domain", "theme",
