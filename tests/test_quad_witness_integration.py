...
class TestQuadWitnessIntegration:
    ...
    def test_psi_shadow_end_to_end(self):
        ...
        assert result["verdict"] == "REJECT"
        assert any(c["type"] == "REVERSIBILITY_CONTRADICTION" 
    ...
    def test_safe_action_allowed(self):
        ...
        # Should pass
        assert result["quad_witness_valid"] == True, \
...
def test_constitutional_compliance():
    ...
    result = build_governance_proof(
        continuity_ok=True, approval_ok=True, human_approve=False,
        public_approval_mode=True, truth_score=0.95, truth_threshold=0.99,
        precedent_count=3, grounding_present=True, revocation_ok=True,
        health_ok=True, omega_ortho=0.04, mode_collapse=False,
        non_violation_status=True,
        proposal="test", agi_result={}, asi_result={}
    )
