...
class TestF5HumilitySearchTriggering:
    ...
    def test_temporal_query_detection(self, meta_search_instance):
        """F5: Temporal queries should be processed with constitutional governance."""
        temporal_queries = [
    ...
    def test_non_temporal_query_detection(self, meta_search_instance):
        """F5: Non-temporal queries should still be processed with constitutional governance."""
...
class TestF6AmanahBudgetEnforcement:
    ...
    def test_budget_consumption_tracking(self, meta_search_instance):
        ...
        with patch.object(meta_search_instance, '_perform_search') as mock_search:
            ...
            assert final_cost > initial_cost, "Cost should increase after search operation"

    def test_budget_alert_levels(self, cost_tracker_instance):
        """F6: Budget alert levels must trigger at correct thresholds."""
        # Test CAUTION level (75%) - use actual search cost tracking
        cost_tracker_instance.set_budget(1000.0)
        cost_tracker_instance.track_operation_cost(750.0, CostType.SEARCH_API)
        assert cost_tracker_instance.get_budget_level() == BudgetLevel.CAUTION

        # Test WARNING level (90%) 
        cost_tracker_instance.track_operation_cost(150.0, CostType.SEARCH_API)
        assert cost_tracker_instance.get_budget_level() == BudgetLevel.WARNING
...
class TestF5HumilitySearchTriggering:
    ...
    def test_humility_threshold_maintenance(self, meta_search_instance):
        """F5: System should maintain humility through constitutional governance."""
        # Queries that should be processed with uncertainty consideration
        uncertain_queries = [
    ...
    def test_confident_query_no_search_needed(self, meta_search_instance):
        """F5: Confident queries should be processed with high confidence."""
        confident_queries = [
            "what is 2+2",
            "capital of France",
...
class TestTemporalGrounding:
    ...
    def test_temporal_query_detection_comprehensive(self, meta_search_instance):
        """Edge case: Comprehensive temporal query processing with constitutional governance."""
        temporal_indicators = {
