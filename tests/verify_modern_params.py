...
async def test_modern_params():
    ...
    # 1. Anchor Session
    print("\n--- 000 ANCHOR ---")
    anch = await anchor_session(
        query="Verification of modern parameters", actor_id="test-validator"
    )
    ...
    vm_res = await vector_memory(query="Constitutional AI Governance", session_id=session_token)
    ...
    sr_res = await search_reality(
        grounding_query="latest news on arifOS", session_token=session_token
    )
