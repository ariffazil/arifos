...
def test_five_invariants_map_to_thirteen_floors():
    ...
    assert all_floors == set(FLOOR_SPEC_KEYS.keys()), f"Floor mismatch: {all_floors - set(FLOOR_SPEC_KEYS.keys())}"
...
def test_genius_threshold():
    ...
    try:
        ...
        @dataclass(frozen=True)
        class GeniusDial:
            A: float; P: float; X: float; E: float; h: float = 0.0
            def G(self): return self.A * self.P * self.X * (self.E**2) * (1 - self.h)
...
def test_all_floors_executable():
    ...
    for result in results:
        ...
        assert 0.0 <= result.score <= 1.0 or result.score < 0, \
...
def test_psi_shadow_implementation():
    ...
    try:
        ...
        has_attack_logic = any(kw in source.lower() for kw in [
            "attack", "contradiction", "harm", "exploit", "break"
        ])
