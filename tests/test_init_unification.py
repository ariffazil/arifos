...
class TestToolSpecCompliance:
    ...
    def test_init_anchor_in_public_specs(self, public_tool_specs):
        """arifos_init is in PUBLIC_TOOL_SPECS with correct modes."""
        spec = next((s for s in public_tool_specs if s.name == "arifos_init"), None)
        assert spec is not None
        assert spec.stage == "000"
    ...
    def test_description_mentions_unification(self, public_tool_specs):
        """Tool description documents session initialization correctly."""
        spec = next((s for s in public_tool_specs if s.name == "arifos_init"), None)
        assert spec is not None
        desc = spec.description
        assert "constitutional session" in desc.lower() or "session" in desc.lower()
        assert "identity" in desc.lower() or "anchor" in desc.lower()
        print("✓ Tool description documents session initialization")
...
class TestImplementationStructure:
    ...
    def test_init_anchor_dispatch_handles_all_modes(self):
        ...
        assert (
            'mode in ("state", "status", "refresh")' in source
            or "mode in ('state', 'status', 'refresh')" in source
        )
...
class TestUnifiedToolSignature:
    ...
    def test_init_anchor_accepts_all_modes_via_payload_and_kwargs(self):
        ...
        assert "raw_input" in params or "kwargs" in params or len(params) >= 1
        print(f"✓ init_anchor signature has flexible ingress: {params}")
