...
def test_federation_registration():
    ...
    coord.register(
        "agent-1",
        lambda: AgentHealth(agent_id="agent-1", status=AgentStatus.HEALTHY, last_check="now"),
    )
...
def test_earth_witness_score():
    coord = FederationCoordinator()
    coord.register(
        "a1", lambda: AgentHealth(agent_id="a1", status=AgentStatus.HEALTHY, last_check="now")
    )
    coord.register(
        "a2", lambda: AgentHealth(agent_id="a2", status=AgentStatus.OFFLINE, last_check="now")
    )
