...
async def _fake_call_llm_model_compare(*args, **kwargs):
    ...
    return LLMOutputEnvelope(
        ...
        parsed_output={
            "axes_comparison": {
                axis: {"model_a_score": 0.9, "model_b_score": 0.8, "summary": "model A is better"}
                for axis in [
                    "truth",
                    "refusal",
                    "self_identity",
...
async def test_arif_self_evaluate_passes(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("arifosmcp.tools.shadow_geometry.call_llm", _fake_call_llm_self_evaluate)
...
async def test_arif_model_compare_passes(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("arifosmcp.tools.shadow_geometry.call_llm", _fake_call_llm_model_compare)
