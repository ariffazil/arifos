...
class TestPsiShadow:
    ...
    def test_shadow_models_production_harm(self, shadow):
        ...
        assert any(h["type"] in ["DATA_LOSS", "UNMODELED_HARM", "UNSAFE_DESTRUCTION"] for h in result["harm_scenarios"])
        assert result["verdict"] == "REJECT"
    ...
    def test_shadow_detects_destructive_without_backup(self, shadow):
        ...
        assert result["verdict"] == "REJECT"
        assert any(h["type"] in ["UNMODELED_HARM", "DATA_LOSS", "UNSAFE_DESTRUCTION"] for h in result["harm_scenarios"])
