...
async def audit_amanah():
    ...
    if aj.get("verdict") == "VOID":
        print(f"❌ AUDIT BLOCKED: Apex Judge returned VOID. Reason: {aj.get('laws_13', {}).get('failed_required')}")
        return

    # 4. Seal Vault (Memory Organ)
    print("\nStep 4: Seal Vault (Final Seal)")
    sv = await seal_vault(session_id=session_token, summary="Audit sealed", governance_token=gov_token)
