...
try:
    ...
    from aaa_mcp.protocol.response import (
        build_init_response, build_seal_response, build_verdict_response
    )
    CORE_AVAILABLE = True
except ImportError:
...
class TestConstitutionalPipeline:
    ...
    def test_vault_seal_has_motto_with_diamond_brain_lock(self):
        ...
        assert response.data.get("motto") == "DITEMPA, BUKAN DIBERI"
        assert response.data.get("motto_english") == "Forged, Not Given"
        assert "💎🧠🔒" in response.data.get("motto_emojis", ""), "VAULT seal must have 💎🧠🔒 emojis"
    ...
    def test_emojis_are_different_for_init_vs_seal(self):
        ...
        assert "💎" in seal_emojis or "🧠" in seal_emojis or "🔒" in seal_emojis, \
    ...
    def test_all_tools_have_capability_descriptions(self):
        ...
        for tool_name in TOOL_GRAPH.keys():
            ...
            assert cap.floors_enforced is not None, \
                f"Tool '{tool_name}' missing floors_enforced"
            assert cap.when_to_use, \
                f"Tool '{tool_name}' missing when_to_use guidance"
    ...
    def test_trinity_forge_is_primary_entrypoint(self):
        ...
        assert "trinity_forge" in WORKFLOW_SEQUENCES["quick_decision"], \
...
class TestAuthenticationRelaxed:
    ...
    def test_auth_token_is_optional(self):
        ...
        if "auth_token" in props:
            ...
            assert "auth_token" not in forge_schema.get("required", []), \
                "auth_token must be optional (not required)"
            # And should accept null
            auth_type = props["auth_token"].get("type", [])
            assert "null" in auth_type or "string" in str(auth_type), \
    ...
    def test_init_gate_no_strict_auth(self):
        ...
        assert ctx.get("authority_token") == "arifos_mcp", \
...
class TestEndToEndFlow:
    ...
    async def test_full_pipeline_flow(self):
        ...
        assert sequence == expected_stages, \
            f"Full analysis sequence mismatch: {sequence}"
