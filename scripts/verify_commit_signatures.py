...
def main():
    ...
    for line in sys.stdin:
        ...
        import subprocess
        result = subprocess.run(['git', 'rev-list', rev_range], capture_output=True, text=True, check=True)
        for commit in result.stdout.splitlines():
            if commit not in signed_commits:
                missing.append(commit)
    if missing:
        sys.stderr.write(f"[verify_commit_signatures] Missing signatures for commits: {', '.join(missing)}\n")
        sys.exit(1)
