...
async def test_schema_flexibility():
    ...
    from arifosmcp.runtime.sessions import _resolve_session_id
    
    ctx = MockContext()

    # Case 1: Auto-generated session_id
    print("\n▶️ Case 1: Missing session_id (Auto-generation)")
    res1 = await metabolic_loop_router(
        query="Test auto-generation",
        dry_run=True,
        ctx=ctx
    )
    ...
    res2 = await metabolic_loop_router(
        query="Test human actor",
        actor_id="human-arif-001",
        dry_run=True,
        ctx=ctx
    )
    # Check if actor_id is preserved in payload/auth_context
    print(f"  ✅ Actor ID accepted: human-arif-001")

    # Case 3: Custom Bot ID (AI LLM)
    print("\n▶️ Case 3: Custom Bot ID (AI LLM)")
    res3 = await metabolic_loop_router(
        query="Test AI bot actor",
        actor_id="bot-gpt4-nexus",
        dry_run=True,
        ctx=ctx
    )
    print(f"  ✅ Bot ID accepted: bot-gpt4-nexus")
    ...
    res4 = await metabolic_loop_router(
        query="Test custom session string",
        dry_run=True,
        ctx=ctx
    )
