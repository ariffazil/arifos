...
def set_covering(required_caps):
    ...
    return {
        ...
        'coverage': {cap: [a['name'] for a in AGENTS if cap in a['caps'] and value(x[a['name']]) == 1] for cap in required_caps}
    }
...
def knapsack(budget, tokens_per_agent):
    ...
    return {
        ...
        'total_tokens': sum(tokens_per_agent.get(a['name'], 0) for a in AGENTS if value(x[a['name']]) == 1),
        'budget': budget,
        'status': LpStatus[prob.status]
    }
...
if __name__ == '__main__':
    ...
    parser.add_argument('--caps', type=str, help='Comma-separated required capabilities (set_cover only)')
