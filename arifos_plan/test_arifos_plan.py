...
test(
    "task-02 in holds",
    any(h["task_id"] == r["plan"]["tasks"][1]["task_id"] for h in r["analysis"]["holds"]),
    True,
)
...
update_status(
    r1["plan"]["plan_id"], "APPROVED", "APPROVED", {"F0": "ok", "F1": "ok"}, "LGTM", "arif-human"
)
...
test(
    ...
    r2["receipts"][0]["decision"] == "APPROVED",
    r2["receipts"][0]["decision"],
)
...
print(
    f"  all pending: {pending['count']}, HIGH: {pending_hi['count']}, irrev: {pending_irrev['count']}"
)

# ─── T6: Sovereign APPROVED → plan APPROVED, receipt SEAL ────────────────────
reset()
r = propose_plan(
    intent={"description": "staging deploy"}, tasks=[], risk_band="HIGH", irreversible=True
)
...
r2 = update_status(
    ...
    "APPROVED",
    {"F0": "ok", "F1": "ok", "L13": "ok"},
    "Staging only, rollback ready",
    "arif-human",
)
test("verdict SEAL", r2["verdict"] == "SEAL", r2["verdict"])
test("plan APPROVED", r2["plan"]["status"] == "APPROVED", r2["plan"]["status"])
test(
    "receipt has floor sigs",
    len(r2["receipt"]["floor_signatures"]) >= 3,
    r2["receipt"]["floor_signatures"],
)
...
reset()
r = propose_plan(
    intent={"description": "needs review"}, tasks=[], risk_band="HIGH", irreversible=True
)
r2 = update_status(
    r["plan"]["plan_id"], "PENDING_APPROVAL", "HOLD", {}, "Need more info", "arif-human"
)
test("verdict HOLD", r2["verdict"] == "HOLD", r2["verdict"])
test(
    "plan still PENDING_APPROVAL", r2["plan"]["status"] == "PENDING_APPROVAL", r2["plan"]["status"]
)
...
r2 = write_execution_receipt(
    ...
    task_id=task_id,
    status="COMPLETED",
    started_at="2026-04-23T01:30:00Z",
    finished_at="2026-04-23T01:30:05Z",
    logs_ref="s3://logs/2026-04-23/git-push-001",
    error="",
    epoch_id="2026-04-23T01Z",
)
test("verdict SEAL", r2["verdict"] == "SEAL", r2["verdict"])
test("execution_receipt returned", "execution_receipt" in r2, True)
test(
    "task status updated",
    r["plan"]["tasks"][0]["status"] == "COMPLETED",
...
check = (
    gk_can_execute(r["plan"]["plan_id"], r["plan"]["tasks"][0]["task_id"])
    if r["plan"]["tasks"]
    else None
)
...
test(
    ...
    "HOLD" in check.get("reason", "") or "approval" in check.get("reason", "").lower(),
    check.get("reason"),
)
...
reset()
r = propose_plan(
    intent={"description": "abort-test"}, tasks=[], risk_band="LOW", irreversible=False
)
