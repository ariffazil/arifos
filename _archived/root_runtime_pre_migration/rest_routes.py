...
from arifos.runtime.public_registry import (
    build_mcp_discovery_json,
    build_server_json,
    public_tool_specs,
)
from arifos.runtime.resources import apex_tools_markdown_table
...
def _build_trinity_matrix(
    ...
    *,
    ...
    # If docker is unavailable (containers list empty), don't flag critical containers as down.
    # Only report missing if we successfully enumerated containers AND they're absent.
    docker_available = bool(containers)
    if docker_available:
...
def _load_welcome_html() -> str:
    ...
    possible_paths = [
        ...
        "/usr/src/app/arifos/runtime/landing_page.html",
        "/usr/src/app/runtime/landing_page.html",
        "/root/ariffazil/arifOS/arifos/runtime/landing_page.html",
    ]
...
<p><strong>DITEMPA BUKAN DIBERI</strong> — Forge deliberately, not hastily.</p>
    ...
    async def health(request: Request) -> Response:
        ...
        try:
            from arifos.runtime.webmcp.live_metrics import get_live_metrics
    ...
    async def metrics_endpoint(request: Request) -> Response:
        ...
        from arifos.runtime.metrics import (
            CONTENT_TYPE_LATEST,
            generate_latest,
    ...
    async def discovery_alias(request: Request) -> Response:
        from arifos.runtime.public_registry import build_mcp_discovery_json
    ...
    async def agent_card_v2(_request: Request) -> Response:
        ...
        from arifos.runtime.a2a.agent_card_v2 import get_arifOS_agent_card
    ...
    async def agent_card_summary(_request: Request) -> Response:
        ...
        from arifos.runtime.a2a.agent_card_v2 import get_axos_summary
    ...
    async def meta_omega(_request: Request) -> Response:
        ...
        from arifos.runtime.a2a.seal_verifier import get_seal_verifier
    ...
    async def meta_omega_violations(_request: Request) -> Response:
        ...
        from arifos.runtime.m01_correlation_auditor import get_auditor
    ...
    async def seal_verify_post(_request: Request) -> Response:
        ...
        from arifos.runtime.a2a.seal_verifier import (
            A2ASealVerifier,
    ...
    # ChatGPT Apps SDK canonical route alias
    @route("/chatgpt/widgets/vault-seal.html", methods=["GET", "OPTIONS"])
    async def vault_seal_widget_chatgpt(request: Request) -> Response:
        """Alias for /widget/vault-seal — ChatGPT Apps SDK expects this path."""
        if request.method == "OPTIONS":
            return Response(status_code=204, headers=widget_headers)
        if not os.path.exists(_widget_file):
            return Response("Widget not found", status_code=404)
    ...
    async def list_resources(request: Request) -> Response:
        ...
        try:
            from arifos.runtime.resources import manifest_resources
    ...
    async def read_resource(request: Request, uri: str) -> Response:
        ...
        try:
            from arifos.runtime.resources import read_resource_content
    ...
    async def a2a_task(request: Request) -> Response:
        ...
        try:
            from arifos.runtime.a2a.server import create_a2a_server
            from arifos.runtime.a2a.models import SubmitTaskRequest, TaskMessage
    ...
    async def webmcp_discovery(request: Request) -> Response:
        ...
        try:
            from arifos.runtime.webmcp.server import create_webmcp_app
...
"""
    ...
    # ── Governance Risk Classification (A-FORGE Bridge) ──────────────────────
    @route("/governance/risk-classify", methods=["POST"])
    async def governance_risk_classify(request: Request) -> Response:
        """Classify a tool or script into T0-T3 risk tier.

        Body: {"tool_name": str, "arguments": dict, "script": str (optional)}
        Returns: {"tier": "T0_INERT|T1_REVERSIBLE|T2_CONTROLLED|T3_IRREVERSIBLE",
                  "requires_approval": bool, "requires_f13": bool,
                  "emits_receipt": bool, "snapshot_required": bool}
        """
        try:
            body = await request.json()
            ...
            # If a raw script is provided, derive the effective tool name from content heuristics
            if not tool_name and script:
                tool_name = _derive_tool_name_from_script(script)

            from arifos.runtime.governance.risk_tiers import classify, RiskTier

            classification = classify(tool_name, arguments)

            return JSONResponse(
                {
                    "tier": classification.tier.value,
                    "requires_approval": classification.requires_approval,
                    "requires_f13": classification.requires_f13,
                    "emits_receipt": classification.emits_receipt,
                    "snapshot_required": classification.snapshot_required,
                    "tool_name": tool_name,
                }
            )
        except Exception as e:
    ...
    def _derive_tool_name_from_script(script: str) -> str:
        """Heuristic: map script content to an effective tool name for risk classification."""
        lowered = script.lower()
        # File system writes
        if "writefile" in lowered or "fs.write" in lowered or "writefile" in lowered:
            return "arifos_forge"
        # Shell execution
        if "exec(" in lowered or "shell" in lowered or "spawn" in lowered:
            return "arifos_diag_substrate"
        # Network posts
        if "fetch(" in lowered or "post(" in lowered or "http" in lowered:
            return "arifos_gateway"
        # Memory writes
        if "memory" in lowered and ("write" in lowered or "store" in lowered or "save" in lowered):
