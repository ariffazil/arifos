"""
arifos/bridge.py — The Harden Bridge
...
from arifos.runtime.contracts import REQUIRES_SESSION
...
async def call_kernel(
    ...
    from arifos.runtime.models import CallerContext as _CallerContext
    ...
    # Every execution entrypoint must normalize intent to a plan before dispatch.
    _intent_for_plan = str(payload.get("query") or payload.get("request") or payload.get("input") or "")
    try:
        ...
        _bridge_plan = normalize_intent_to_plan(
            intent=_intent_for_plan,
            session_id=session_id,
            actor_id=claimed_actor_id,
            risk_tier=payload.get("risk_tier", "medium"),
        )
        _bridge_plan = evaluate_plan_against_governance(_bridge_plan)

        # Inject plan metadata into payload for downstream observability
        payload["_plan_metadata"] = plan_to_execution_manifest(_bridge_plan)
        payload["_plan_id"] = _bridge_plan.id

        # 888 HOLD gate for non-dry-run execution paths
        if (
            _bridge_plan.governance_verdict == "HOLD"
            and not dry_run
            and canonical_name not in {"search_reality", "ingest_evidence", "trace_replay", "arifos_health", "check_vital"}
        ):
            return {
                "ok": False,
                "tool": canonical_name,
                ...
                "error": "PLAN_888_HOLD",
                "summary": (
                    f"Plan '{_bridge_plan.id}' blocked by governance gate. "
                    f"Irreversibility={_bridge_plan.irreversibility_score}. "
                    "Human approval required before execution."
                ),
                "plan": plan_to_execution_manifest(_bridge_plan),
                "session_id": session_id,
            }
    except Exception as _plan_err:
        # Plan gate failure is non-fatal — log and continue with degraded governance
        import logging

        logging.getLogger(__name__).warning(f"[BRIDGE] Plan gate error: {_plan_err}")
    ...
    try:
        ...
        if canonical_name == "init_anchor":
        ...
        elif tool_name == "agi_reason":
            # Task Δ: Use the decision packet generated by run_agi_mind
            ...
            from arifos.runtime.tools_internal import reality_atlas
            ...
            from arifos.runtime.orchestrator import metabolic_loop
