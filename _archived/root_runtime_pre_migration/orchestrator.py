"""
arifos/runtime/orchestrator.py — The arifOS Metabolic Loop Orchestrator
...
async def handle_pns_search(query: str, session_id: str) -> RuntimeEnvelope:
    ...
    from arifos.runtime.tools import reality_compass
...
async def handle_pns_health(session_id: str) -> RuntimeEnvelope:
    ...
    from arifos.runtime.tools import check_vital
...
async def handle_pns_orchestrate(task: str, session_id: str) -> RuntimeEnvelope:
    ...
    from arifos.tools.agentzero_tools import agentzero_engineer
...
async def handle_pns_floor(input_data: Any, session_id: str) -> RuntimeEnvelope:
    ...
    from arifos.runtime.tools import audit_rules
...
async def handle_pns_redteam(candidate: str, session_id: str) -> RuntimeEnvelope:
    ...
    from arifos.tools.agentzero_tools import agentzero_validate
...
async def run_stage(
    ...
    """
    from arifos.runtime.tools import (
    ...
    try:
        ...
        if stage_id == Stage.FORGE_777.value:
            from arifos.runtime.tools import agi_asi_forge_handler
...
async def metabolic_loop(
    ...
    **kwargs,
    ...
    from arifos.runtime.sessions import _resolve_session_id as _normalize_session_id
    ...
    # ── PLAN RECONCILER: VOID anonymous abandoned plans (F11) ──────────────
    try:
        from core.kernel.planner import reconcile_abandoned_plans

        _reconciled = reconcile_abandoned_plans(max_age_minutes=5.0)
        if _reconciled:
            logger.info(f"[RECONCILER] Cleared {_reconciled} anonymous abandoned plans")
    except Exception as _rec_err:
        logger.debug(f"[RECONCILER] Sweep error (non-fatal): {_rec_err}")

    # ── PLANNING ORGAN: Intent → Plan → Governance Gate ──────────────────
    # Create plan BEFORE dry_run or any other gates so it is always present.
    ...
    execution_plan = normalize_intent_to_plan(
        intent=query,
        session_id=current_session_id,
        actor_id=actor_id,
        risk_tier=risk_tier,
    )
    execution_plan = evaluate_plan_against_governance(execution_plan)
    plan_metadata = plan_to_execution_manifest(execution_plan)

    if dry_run:
        from arifos.runtime.models import AuthContext
    ...
    # 888 HOLD gate: high-risk / irreversible plans require explicit approval
    if execution_plan.governance_verdict == "HOLD" and allow_execution:
        logger.warning(
            f"[METABOLIC_LOOP] 888_HOLD for plan {execution_plan.id}: "
            f"score={execution_plan.irreversibility_score}"
        )
        return {
            "ok": False,
            "tool": tool_name,
            ...
            "errors": [
                {
                    "message": (
                        f"Plan '{execution_plan.id}' blocked by governance gate. "
                        f"Irreversibility={execution_plan.irreversibility_score}. "
                        "Human approval required before execution."
                    ),
                    "code": "PLAN_888_HOLD",
                }
            ],
            "trace": {"plan_id": execution_plan.id, "plan": plan_metadata},
        }
    ...
    try:
        ...
        if len(plan) >= 3:
            try:
                ...
                from arifos.runtime.dag_executor import ConstitutionalDAGExecutor
                from arifos.runtime.irreversibility import AmanahIrreversibilityScorer
                ...
                for idx, stage_id in enumerate(plan):
                    ...
                    dag_exec.add_node(
                        ...
                        dependencies=deps,
                        plan_id=plan_metadata.get("plan_id", "") if "plan_metadata" in locals() else "",
                    )
        ...
        from arifos.runtime.metrics import compute_integrity_telemetry
        ...
        out.update(
            {
                ...
                # Plan metadata (repaired Planning Organ)
                "plan": plan_metadata,
            }
        )
