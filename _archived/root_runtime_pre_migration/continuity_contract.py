"""
arifos/runtime/continuity_contract.py — Shared cross-tool continuity contract.
...
from arifos.runtime.middleware.invariant_enforcement import enforce_invariants
from arifos.runtime.models import ClaimStatus, RuntimeEnvelope, Verdict
...
def seal_runtime_envelope(
    ...
    *,
    ...
    CONSTITUTIONAL GUARD (2026-04-20):
    All envelopes pass through constitutional_guard before sealing.
    Shell-SEAL is overridden with honest verdict based on floor metrics.
    """
    # ── Constitutional Guard Interception ────────────────────────────────────
    # Dump envelope to dict, run guard, apply honest verdict back to model.
    try:
        envelope_dict = envelope.model_dump(mode="json") if hasattr(envelope, "model_dump") else dict(envelope.__dict__)
        guarded = constitutional_guard(tool_id, envelope_dict)

        # Apply guarded verdict back to envelope model
        new_verdict_str = guarded.get("verdict")
        if new_verdict_str and hasattr(Verdict, new_verdict_str):
            new_verdict = getattr(Verdict, new_verdict_str)
            if envelope.verdict != new_verdict:
                if str(envelope.verdict) == "SEAL" and new_verdict_str != "SEAL":
                    logger.warning(
                        "ConstitutionalGuard: Shell-SEAL intercepted for %s → %s",
                        tool_id,
                        new_verdict_str,
                    )
                envelope.verdict = new_verdict

        # Attach guard metadata to policy
        envelope.policy = envelope.policy or {}
        if isinstance(envelope.policy, dict):
            envelope.policy["floor_results"] = guarded.get("floor_results", {})
            envelope.policy["missing_output_fields"] = guarded.get("missing_output_fields", [])
            envelope.policy["reasoning_hash"] = guarded.get("reasoning_hash", "")
            envelope.policy["constitutional_guard_version"] = guarded.get("constitutional_guard_version", "")
    except Exception as exc:
        # Guard failure must not break the pipeline — degrade to PARTIAL
        logger.error("ConstitutionalGuard failed for %s: %s", tool_id, exc)
        envelope.verdict = Verdict.PARTIAL
        envelope.policy = envelope.policy or {}
        if isinstance(envelope.policy, dict):
            envelope.policy["constitutional_guard_error"] = str(exc)
            envelope.policy["constitutional_guard_version"] = "v1-error"

    # ── Invariant Enforcement (AGI-Level Intelligence) ─────────────────────────
    # Run invariant checks after constitutional guard.
    # Downgrade verdict to PARTIAL if invariants fail.
    try:
        envelope_dict2 = envelope.model_dump(mode="json") if hasattr(envelope, "model_dump") else dict(envelope.__dict__)
        invariant_enforced = enforce_invariants(tool_id, envelope_dict2, input_payload)
        invariant_failures = invariant_enforced.get("invariant_failures", [])

        if invariant_failures:
            logger.warning(
                "Invariant enforcement failures for %s: %s",
                tool_id,
                invariant_failures,
            )
            ...
            envelope.policy = envelope.policy or {}
            if isinstance(envelope.policy, dict):
                envelope.policy["invariant_failures"] = invariant_failures
                envelope.policy["invariant_enforcement_version"] = "v1"

        meta_intel = invariant_enforced.get("meta_intelligence", {})
        envelope.payload = getattr(envelope, "payload", {}) or {}
        if isinstance(envelope.payload, dict):
            envelope.payload["meta_intelligence"] = meta_intel

    except Exception as exc:
        logger.error("InvariantEnforcement failed for %s: %s", tool_id, exc)
        if envelope.verdict == Verdict.SEAL:
            envelope.verdict = Verdict.PARTIAL
        envelope.policy = envelope.policy or {}
        if isinstance(envelope.policy, dict):
            envelope.policy["invariant_enforcement_error"] = str(exc)
            envelope.policy["invariant_enforcement_version"] = "v1-error"

    # ── Original short-circuit check (after guard) ───────────────────────────
