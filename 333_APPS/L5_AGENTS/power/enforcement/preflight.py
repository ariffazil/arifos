"""POWER preflight gate entrypoint."""
...
def preflight(profile: dict[str, Any], context: dict[str, Any]) -> PreflightResult:
    ...
    combined = "\n".join(part for part in (query, draft) if part).strip()
    ...
    if omega_0 > elevated:
        reasons.append("F7 Humility elevated Omega_0")

    return PreflightResult("SABAR" if reasons else "SEAL", omega_0, tuple(reasons))
