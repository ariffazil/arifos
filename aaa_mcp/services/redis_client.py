...
REDIS_URL = os.getenv("REDIS_URL", "redis://default:SZWGzoMRhSeJGZvaKvMoSZYzuPrzofyw@redis.railway.internal:6379")
...
async def init_gate(query: str, session_id: Optional[str] = None) -> dict:
    ...
    # Load previous state (survives ephemeral execution)
    state = vault.load(session_id)
    ...
    vault.save(session_id, state)
    
    return {"verdict": "SEAL", "session_id": session_id, "state_loaded": bool(state)}
"""
