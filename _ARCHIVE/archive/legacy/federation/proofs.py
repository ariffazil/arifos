...
class ZKConstitutionalProof:
    ...
    def setup(self, floors: List[str]) -> Tuple[bytes, bytes]:
        ...
        self.verification_key = hashlib.sha256(self.proving_key + b"verification").digest()

        self.setup_complete = True

        return self.proving_key, self.verification_key

    def _create_inequality_constraint(
        self, threshold: float, negate: bool = False
    ) -> CircuitConstraint:
    ...
    def _create_range_constraint(self, lower: float, upper: float) -> CircuitConstraint:
        ...
        return CircuitConstraint(a=[(0, 1.0), (1, -1.0)], b=[(2, 1.0)], c=[(3, upper - lower)])
