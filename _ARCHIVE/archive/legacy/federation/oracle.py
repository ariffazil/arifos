...
class RealityOracle:
    ...
    def instantiate(
        ...
        if verdict == Verdict.SEAL:
            merkle_root = self._commit_to_ledger(verdict, W3, G, floor_results)
    ...
    def _commit_to_ledger(
        ...
        if self.human_witness:
            signatures["human"] = hashlib.sha256(b"human" + str(time.time()).encode()).hexdigest()[
                :32
            ]
        if self.ai_witness:
            signatures["ai"] = hashlib.sha256(b"ai" + str(time.time()).encode()).hexdigest()[:32]
        if self.earth_witness:
            signatures["earth"] = hashlib.sha256(b"earth" + str(time.time()).encode()).hexdigest()[
                :32
            ]
    ...
    def verify_chain(self, cid: str) -> Dict:
        ...
        verification = {"cid": cid, "chain_length": len(chain), "verified": True, "links": []}
