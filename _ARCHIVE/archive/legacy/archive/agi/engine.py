...
from .hardening import run_pre_checks, run_post_checks, cleanup_session, HardeningResult, RiskLevel
...
class SessionManager:
    ...
    def cleanup_expired(self):
        ...
        now = time.time()
        expired = [
            sid for sid, data in self._engines.items() if now - data["created_at"] > self._ttl
        ]
        for sid in expired:
...
class AGIResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "error": self.error,
            "risk_level": (
                self.risk_level.value if hasattr(self.risk_level, "value") else str(self.risk_level)
            ),
            "delta_bundle": (
                self.delta_bundle.to_dict()
                if hasattr(self.delta_bundle, "to_dict")
                else str(self.delta_bundle)
            ),
...
class AGIEngine:
    ...
    async def execute(
        ...
        if not self.circuit_breaker.can_execute():
            return self._build_blocked_result(
                exec_id, start_time, None, "Circuit breaker OPEN - too many recent failures"
            )
        ...
        try:
            ...
            if not is_safe:
                ...
                return self._build_blocked_result(exec_id, start_time, None, error_msg)
            ...
            if sense_data.get("lane") == "VOID":
                self.circuit_breaker.record_failure()
                return self._build_blocked_result(
                    exec_id, start_time, hardening, sense_data.get("error", "Sense phase failed")
                )
            ...
            best_result = max(
                parallel_results,
                key=lambda x: x.get("precision_weighted_confidence", x.get("confidence", 0)),
            )
            ...
            forge_data = await self.forge_engine.forge_response(
                {
                    ...
                    "confidence": best_result.get(
                        "precision_weighted_confidence", best_result.get("confidence", 0.9)
                    ),
    ...
    async def _run_parallel_think_async(
        ...
        try:
            results = await asyncio.wait_for(
                asyncio.gather(*tasks, return_exceptions=True), timeout=30.0  # 30 second timeout
            )
    ...
    async def _think_path(self, mode: str, query: str, lane: str) -> Dict[str, Any]:
        ...
        try:
            think_data = await self.think_engine.reason(
                {"query": f"[{mode.upper()}] {query}", "lane": lane, "mode": mode}
            )
    ...
    def _build_blocked_result(
        ...
        bundle = DeltaBundle(
            session_id=session_id, vote=EngineVote.VOID, vote_reason=f"Blocked: {block_reason}"
        )
    ...
    def _build_error_result(self, session_id: str, exec_time_ms: float, error: str) -> AGIResult:
        ...
        bundle = DeltaBundle(
            session_id=session_id, vote=EngineVote.VOID, vote_reason=f"Engine Error: {error}"
        )
        if hasattr(bundle, "seal"):
            bundle = bundle.seal()
