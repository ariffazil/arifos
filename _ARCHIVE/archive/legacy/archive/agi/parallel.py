...
class ParallelHypothesisResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "think_output": (
                self.think_output.to_dict()
                if hasattr(self.think_output, "to_dict")
                else str(self.think_output)
            ),
            "reason_output": (
                self.reason_output.to_dict()
                if hasattr(self.reason_output, "to_dict")
                else str(self.reason_output)
            ),
        }
...
class ParallelHypothesisMatrix:
    ...
    def generate_parallel_hypotheses(
        ...
        tasks = [self._execute_hypothesis_path(mode, sense_output, context) for mode in self.modes]
