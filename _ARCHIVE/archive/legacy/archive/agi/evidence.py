...
def compute_precision_weighted_update(
    prior_conf: float, evidence_conf: float, prediction_error: float
) -> float:
    """
...
class EvidenceKernel:
        ...
        for i, query in enumerate(search_queries):
            try:
                result = self._search_and_extract(
                    query["query"], query["source"], query["priority"]
                )
    ...
    def _search_and_extract(
        ...
        try:
            ...
            bundle = EvidenceBundle(query=query, source=source, facts=facts, confidence=confidence)
        ...
        for result in search_results:
            ...
            for num in numbers:
                fact = ParsedFact(
                    ...
                    content=(
                        f"{num}% performance improvement"
                        if "improve" in snippet
                        else f"{num}% impact"
                    ),
    ...
    def _compute_confidence(self, facts: List[ParsedFact], source: str, priority: float) -> float:
        ...
        source_multiplier = {"peer_review": 1.0, "local_asean": 0.95, "recent": 0.85}.get(
            source, 0.8
        )
    ...
    def get_evidence_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "confidence_boost": (
                sum(b.confidence for b in self.evidence_registry) / len(self.evidence_registry)
