...
class ThermodynamicDashboard:
    ...
    def _trigger_humility_violation(self, omega_0: float):
        ...
        logger.warning(
            f"[F6 Violation] Ω₀ = {omega_0:.4f} (outside [0.03, 0.05]) - Session: {self.session_id}"
        )
    ...
    def _trigger_truth_warning(self, confidence: float):
        ...
        logger.warning(
            f"[F2 Warning] Confidence = {confidence:.4f} (< 0.95) - Session: {self.session_id}"
        )
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if stats["average_delta_s"] > -0.1:
            recommendations.append(
                "Consider adding more evidence or deeper reasoning to increase entropy reduction"
            )
