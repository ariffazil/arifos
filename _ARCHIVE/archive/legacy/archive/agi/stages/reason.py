    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "reasoning": {
                ...
                "inference_steps": (
                    self.reasoning_tree.inference_steps if self.reasoning_tree else []
                ),
...
def synthesize_reasoning_tree(sense: SenseOutput, think: ThinkOutput) -> ReasoningTree:
    """
    ...
    if think.conservative:
        inference_steps.append(f"[Conservative] {think.conservative.content[:150]}")

    # Step 2: Exploratory analysis
    if think.exploratory:
        inference_steps.append(f"[Exploratory] {think.exploratory.content[:150]}")

    # Step 3: Adversarial challenge
    if think.adversarial:
        inference_steps.append(f"[Adversarial] {think.adversarial.content[:150]}")

    # Step 4: Synthesis
    inference_steps.append("[Synthesis] Integrating all three perspectives into unified response")
...
def detect_contradictions(hypotheses: List[Hypothesis]) -> List[str]:
    ...
    if len(hypotheses) >= 2:
        ...
        if conservative and exploratory:
            ...
            if (
                "should" in conservative.content.lower()
                and "should not" in exploratory.content.lower()
            ):
...
def compute_f2_truth_score(sense: SenseOutput, reasoning_tree: ReasoningTree) -> float:
    """
...
def compute_all_floor_scores(
    sense: SenseOutput, think: ThinkOutput, reasoning_tree: ReasoningTree, delta_s: float
) -> AGIFloorScores:
    """
...
def cast_agi_vote(
    ...
    if not f7_in_band:
        failures.append(
            f"F7 Humility Ω₀={floor_scores.F7_humility:.3f} outside [{OMEGA_0_MIN}, {OMEGA_0_MAX}]"
        )
    ...
    if not f13_pass:
        ...
        return (
            EngineVote.UNCERTAIN,
            f"AGI UNCERTAIN: F13 Curiosity {floor_scores.F13_curiosity} < {F13_PATHS_MIN}",
        )
...
def execute_stage_333(
    ...
    s_before, s_after, delta_s = compute_delta_s(sense_output.raw_query, reasoning_tree)
    output.entropy_before = s_before
    output.entropy_after = s_after
    output.delta_s = delta_s

    # 3. Compute all floor scores
    floor_scores = compute_all_floor_scores(sense_output, think_output, reasoning_tree, delta_s)
...
def _apply_precision_updates(think: ThinkOutput, facts: List[ParsedFact]):
    ...
    for h in hypotheses:
        ...
        new_conf = compute_precision_weighted_update(
            prior_conf=h.confidence, evidence_conf=avg_evidence_conf, prediction_error=error
        )
