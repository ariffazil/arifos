...
from codebase.engines.apex.apex_components import JudiciaryValidator, TrinityConsensusEngine
...
class APEXRoom:
    ...
    async def run_trinity_sync(self) -> MergedBundle:
        ...
        if not delta or not omega:
            raise ValueError(
                f"Missing bundles for Trinity Sync: Delta={bool(delta)}, Omega={bool(omega)}"
            )
        ...
        # 2. Map Floor Scores
        merged = MergedBundle(
            session_id=self.session_id, delta_bundle=delta, omega_bundle=omega, consensus=consensus
        )
...
async def test_apex_trinity_sync():
    ...
    delta = DeltaBundle(
        session_id=session_id, query="test", draft="test", truth_score=0.99, vote=EngineVote.SEAL
    )
    omega = OmegaBundle(
        session_id=session_id, weakest_stakeholder="User", empathy_kappa=0.99, vote=EngineVote.SEAL
    )
