...
async def _stage_444_impact(agi_output: AgiOutput) -> float:
    ...
    thought_dicts = [{"confidence": node.confidence} for node in agi_output.thoughts]
...
async def core_asi(
    ...
    stakeholder_impact = _compute_stakeholder_impact(
        query, agi_output, peace_squared, kappa_r_value
    )

    # Build ASI metrics
    metrics = AsiMetrics(peace_squared=peace_squared, kappa_r=kappa_r_value, c_dark=c_dark)
