"""
arifosmcp.transport/asgi.py — ASGI entrypoint for FastMCP HTTP hosting (Prefect Horizon, Railway, etc.)

Exports:
  - `app`: Starlette ASGI application with MCP mounted at `/mcp` and health at `/health`.

Notes:
  - Uses the existing unified registry in `arifosmcp.transport.server` (tools/resources/prompts/templates).
...
def create_aaa_mcp_server():
    """Compatibility shim preserving old factory name in arifosmcp.transport transport."""
