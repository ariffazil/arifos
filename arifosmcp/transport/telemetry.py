"""
arifosmcp.transport/telemetry.py — Hardened FastMCP Telemetry Integration
...
from arifosmcp.transport.infrastructure.monitoring import PipelineMetrics, get_metrics_collector
from arifosmcp.transport.observability import (
    record_floor_score,
...
def record_pipeline_metrics(
    ...
    if OTEL_AVAILABLE:
        ...
        with tracer.start_as_current_span("arifos.pipeline.complete") as span:
            ...
            span.set_attribute(
                "arifos.floors_failed", sum(1 for v in floors_checked.values() if not v)
            )
