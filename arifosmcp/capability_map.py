...
def build_llm_context_map() -> dict[str, Any]:
    """Build LLM context map from CAPABILITY_MAP for discovery metadata."""
    def _serialize_capability(cap):
        if hasattr(cap, "__dataclass_fields__"):
            return {k: _serialize_capability(v) for k, v in cap.__dict__.items()}
        if isinstance(cap, (list, tuple)):
            return [_serialize_capability(x) for x in cap]
        if isinstance(cap, dict):
            return {k: _serialize_capability(v) for k, v in cap.items()}
        return cap

    return {
        "schema": "llm-context-map/v1",
        "capabilities": _serialize_capability(CAPABILITY_MAP),
        "mega_tools": _serialize_capability(list(MEGA_TOOLS)),
    }
