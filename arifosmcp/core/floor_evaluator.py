...
from arifosmcp.core.threat_engine import IrreversibilityLevel, ThreatAssessment, ThreatCategory
    ...
    def evaluate(cls, context: Any, threat: ThreatAssessment) -> FloorResult:
        ...
        if effective_irreversibility.value >= IrreversibilityLevel.HIGH.value:
            if not getattr(context, "ack_irreversible", False):
                failed.append("F01")
                reasons["F01"] = (
                    f"Irreversible action requires ack_irreversible=True "
                    f"(level={effective_irreversibility.name})"
                )
                ...
                if context.tool_name == "arif_mind_reason" and context.mode == "plan_approve":
                    if not ack:
                        ...
                        reasons["F13_VIOLATION"] = (
                            "F13 SOVEREIGN: AI self-approval requires explicit sovereign ack"
                        )
                elif not ack:
                    failed.append("F13")
                    wt = getattr(context, "witness_type", "ai")
                    reasons["F13"] = f"F13: human_witness or ack required (wt={wt}, ack={ack})"
