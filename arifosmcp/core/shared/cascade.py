...
Floors enforced: L10 ELEGANCE, L13 SOVEREIGN, F1 AMANAH
...
Tier 3 (sovereign=True) MUST succeed — failure raises SovereigntyFloorBreach (L13 violation).
"""
...
class TierConfig:
    ...
    Args:
        ...
        sovereign: If True, this is Tier 3 (sovereignty floor). Failure here
                   raises SovereigntyFloorBreach exception (L13 violation).
...
class SovereigntyFloorBreach(Exception):
    ...
    This is a CONSTITUTIONAL VIOLATION (L13 SOVEREIGN).
...
async def tiered_call(
    ...
    Raises:
        SovereigntyFloorBreach: If the sovereign tier (tier 3) fails. This is L13 violation.
    ...
    if sovereign_count == 0:
        raise ValueError(
            "No sovereign tier defined. L13 SOVEREIGN requires exactly one tier with sovereign=True."
        )
    if sovereign_count > 1:
        raise ValueError(
            f"Multiple sovereign tiers defined ({sovereign_count}). "
            "L13 SOVEREIGN requires exactly ONE sovereignty floor."
        )
    ...
    for idx, tier in enumerate(tiers):
        ...
        try:
            ...
            # If this is the sovereign tier, CANNOT fall through — raise L13 violation
            if tier.sovereign:
                raise SovereigntyFloorBreach(
                    f"[L13 SOVEREIGN VIOLATION] Tier {tier_num} ({tier.name}) is the sovereignty floor "
