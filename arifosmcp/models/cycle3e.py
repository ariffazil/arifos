...
class SourceAttribution(BaseModel):
    ...
    search_engine: SearchEngine | None = Field(
        default=None, description="Search engine used to find this source"
    )
    fetch_method: FetchMethod | None = Field(
        default=None, description="Method used to fetch this source"
    )
    credibility_score: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Credibility rating (0-1)"
    )
...
class EvidenceBundle(BaseModel):
    ...
    created_at: datetime = Field(
        default_factory=datetime.utcnow, description="When this bundle was created"
    )
    exploration_phase: str = Field(
        default="complete", description="Status of exploration for this bundle"
    )

    def compute_content_hash(self) -> str:
        """Compute hash of aggregated content for verification."""
        return hashlib.sha256(self.aggregated_content.encode()).hexdigest()
...
class ExplorationPhase(BaseModel):
    ...
    evidence_bundle: EvidenceBundle | None = Field(
        default=None, description="Resulting evidence bundle"
    )
...
class EntropyPhase(BaseModel):
    ...
    metabolization_score: float = Field(
        default=0.0, ge=0.0, le=1.0, description="How well evidence was metabolized"
    )
    uncertainty_delta: float = Field(
        default=0.0, description="Change in uncertainty (negative = more certain)"
    )
    start_time: datetime = Field(default_factory=datetime.utcnow)
    end_time: datetime | None = Field(default=None)
    status: str = Field(default="in_progress")
...
class EurekaPhase(BaseModel):
    ...
    tri_witness: list[WitnessStatement] = Field(
        default_factory=list, description="Tri-Witness statements"
    )
    synthesis: str = Field(default="", description="Synthesized conclusion")
    verdict: str | None = Field(default=None, description="Final verdict")
    confidence_final: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Final confidence score"
    )
