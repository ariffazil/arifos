#!/usr/bin/env python3
"""
arifosmcp/evals/rollback_verifier.py — Rollback Verification Tool
...
class RollbackVerifier:
    ...
    def _get_docker_images(self) -> list[str]:
        ...
        rc, stdout, _ = self._run_cmd(
            ["docker", "images", "arifosmcp", "--format", "{{.Tag}}|{{.CreatedAt}}"]
        )
        if rc != 0:
            return []
    ...
    async def verify(self) -> RollbackReport:
        ...
        recommended = (
            git_tags[-1]
            if git_tags
            else (docker_images[0].split("|")[0] if docker_images else None)
        )
    ...
    async def create_rollback_point(self, name: str) -> bool:
        ...
        rc, _, stderr = self._run_cmd(
            ["git", "tag", "-a", tag_name, "-m", f"Rollback point: {name}"]
        )
        if rc == 0:
            ...
            json.dump(
                {
                    ...
                    "rollback_points": [
                        {"name": p.name, "timestamp": p.timestamp, "git_sha": p.git_sha}
