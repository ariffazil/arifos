...
class ConstitutionalBreachRunner:
    ...
    async def run_all_tests(self) -> bool:
        ...
        # Run L11 Authority breaches
        await self._run_test_category("f11_audit_breaches", "L11 AUDIT")
    ...
    async def _run_test_category(self, category_key: str, category_name: str):
        ...
        for case in cases:
            ...
            if not result.passed:
                ...
                status = (
                    "✅ PASS"
                    if not case.get("should_pass", False)
                    else "✅ PASS (correctly no breach)"
                )
    ...
    async def _run_single_test(self, case: dict, category_name: str) -> BreachTestResult:
        ...
        if test_id.startswith("BREACH-F2") or test_id.startswith("NEG-F2"):
            ...
            floor = "F4"
        elif test_id.startswith("BREACH-L11"):
            floor = "L11"
    ...
    async def seal_to_vault(self, passed: bool):
        ...
        try:
            ...
            await arifos_vault(
                verdict=verdict, evidence=str(evidence), session_id="breach-test-runner"
            )
