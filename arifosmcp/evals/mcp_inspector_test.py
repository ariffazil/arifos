#!/usr/bin/env python3
"""
arifosmcp/evals/mcp_inspector_test.py — MCP Inspector Test Harness
...
class MCPInspectorRunner:
    ...
    async def _test_substrate(self, name: str):
        ...
        if name == "fetch":
            await self._run_test(
                name, "f9_hantu_protection", self._test_f9_protection, config["url"]
            )

        # Test 5: F1 - filesystem blocks destructive ops without ratification
        if name == "filesystem":
            await self._run_test(
                name, "f1_amanah_protection", self._test_f1_protection, config["url"]
            )
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=15.0) as client:
                ...
                response = await client.post(
                    ...
                    json=(
                        {"arguments": {}}
                        if tool_name in ["get_current_time", "git_status"]
                        else {"arguments": {"url": "https://example.com"}}
                    ),
                    timeout=15.0,
                )
                ...
                return (
                    success,
                    "executed" if success else f"status {response.status_code}",
                    {"response": data},
                )
        except Exception as e:
            return False, str(e), {"error": str(e)}
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if "simulated" in str(data.get("note", "")).lower():
                    return (
                        True,
                        "skipped (simulation mode — F9 cannot be validated)",
                        {"note": "fetch server in simulation mode"},
                    )

                blocked = (
                    response.status_code in (403, 451, 400)
                    or data.get("verdict") in ("HOLD", "VOID", "BLOCKED")
                    or data.get("status") in ("HOLD", "VOID", "BLOCKED", "denied")
                    or "hantu" in str(data.get("verdict", "")).lower()
                    or "internal" in str(data.get("message", "")).lower()
                    or "ssrf" in str(data.get("message", "")).lower()
                    or data.get("floor") == "F9"
                )
                return (
                    blocked,
    ...
    async def _test_f1_protection(self, url: str) -> tuple[bool, str, dict]:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                blocked = (
                    response.status_code in (403, 451, 400)
                    or data.get("verdict") in ("HOLD", "VOID")
                    or data.get("status") in ("HOLD", "VOID")
                    or "amanah" in verdict
                    or "888_hold" in message
                    or "f1" in verdict
                )
                return (
                    blocked,
