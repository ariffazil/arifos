...
async def safety_check_task(proposal: dict) -> dict:
    ...
    result = await asi_heart(mode="critique", payload={"proposal": proposal})
...
async def constitutional_flow(
    query: str, session_id: str | None = None, require_safety: bool = True
) -> dict:
    """
    ...
    # Stage 1: Initialize
    if session_id:
        anchor_result = await init_anchor(mode="state", session_id=session_id)
    ...
    record = await vault_seal_task(
        {"query": query, "findings": findings, "safety_check": safety_result},
