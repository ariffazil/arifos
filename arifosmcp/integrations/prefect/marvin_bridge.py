...
class ArifOSAgent(Agent):
    ...
    async def run(self, task: Task) -> Any:
        ...
        validation = await apex_soul(
            mode="validate",
            payload={"input": task.instructions, "output": result, "agent": self.name},
        )
...
async def governed_ai_task(
    ...
    if enable_critique:
        validation = await apex_soul(
            mode="validate", payload={"input": instructions, "output": result}
        )
...
async def governed_extract(
    text: str, target_type: type, instructions: str = "", enable_critique: bool = True
) -> Any:
    """
