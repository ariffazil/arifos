...
ACTION_CLASSES = ["observer", "interpreter", "maker", "messenger", "mutator", "destroyer", "sovereign"]
...
def _art_check(goal: str, action_class: str | None, blast_radius: str | None) -> dict[str, Any]:
    ...
    radius = blast_radius or ("high" if cls in {"destroyer", "mutator", "sovereign"} else "medium" if cls in {"messenger", "maker"} else "low")

    power = {"observer": 1, "interpreter": 1, "maker": 2, "messenger": 3, "mutator": 4, "destroyer": 5, "sovereign": 6}.get(cls, 2)
...
def _preflight_health() -> dict[str, Any]:
    ...
    try:
        ...
        return {"name": "arifOS health", "pass": status == "healthy", "detail": status, "data": data}
...
def _preflight_vault() -> dict[str, Any]:
    ...
    try:
        data = http_get_json(url, timeout=5)
        return {"name": "VAULT999 writer", "pass": data.get("status") == "healthy", "detail": data.get("status", "unknown"), "data": data}
    except CliError as exc:
...
def _preflight_git() -> dict[str, Any]:
    ...
    clean = out == ""
    return {"name": "git working tree", "pass": clean, "detail": "clean" if clean else f"{len(out.splitlines())} modified files", "data": {"dirty_files": out.splitlines() if not clean else []}}


def _preflight_lease() -> dict[str, Any]:
    """Placeholder for lease validity check (A-FORGE integration)."""
    return {"name": "A-FORGE lease", "pass": True, "detail": "not yet implemented — manual check required", "data": {}}
...
def run_check(args: list[str]) -> int:
    ...
    parser = argparse.ArgumentParser(prog="arifos check", description="ART + preflight before action.")
    parser.add_argument("--goal", required=True, help="What you intend to do.")
    parser.add_argument("--class", dest="action_class", choices=ACTION_CLASSES, help="Action class (auto-detected if omitted).")
    parser.add_argument("--blast-radius", choices=BLAST_RADIUS, help="Blast radius (auto-detected if omitted).")
    parser.add_argument("--json", action="store_true", help="Output JSON.")
    parser.add_argument("--full", action="store_true", help="Include extra probes (security audit).")
    ...
    result = {
        ...
        "art": art,
        "preflight": checks,
        "recommendation": "PROCEED" if verdict == "SEAL" else "STOP — resolve holds before execution",
    }
    ...
    if parsed.json:
        ...
        print(f"action_class: {art['action_class']} | blast_radius: {art['blast_radius']} | ceremony: {art['ceremony']}")
        print("preflight:")
        for c in checks:
