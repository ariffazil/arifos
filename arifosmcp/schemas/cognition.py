...
class ThinkingStep(BaseModel):
    ...
    hypothesis_formed: str | None = Field(
        default=None, description="New hypothesis formed at this step"
    )
    hypothesis_rejected: str | None = Field(
        default=None, description="Hypothesis rejected at this step"
    )
...
class ThinkingSequence(BaseModel):
    ...
    epistemic_humility_maintained: bool = Field(
        default=True, description="Did the AI maintain appropriate epistemic humility?"
    )
...
class ResourceMetrics(BaseModel):
    ...
    reasoning_efficiency: float = Field(
        ge=0.0, le=1.0, description="Confidence gained per unit of resource (0-1 scale)"
    )
    confidence_per_token: float = Field(
        default=0.0, description="Confidence improvement per token consumed"
    )
    insight_per_joule: float | None = Field(
        default=None, description="Epistemic value per joule of energy (if calculable)"
    )
...
class RealityAnchor(BaseModel):
    ...
    irreversibility_flag: bool = Field(
        default=False, description="Does evidence suggest irreversible consequences?"
    )
...
class EvidenceOutput(BaseModel):
    ...
    thinking_sequence: ThinkingSequence | None = Field(
        default=None, description="Sequential thinking process if thinking_depth > 0"
    )

    # Resource accounting
    resource_metrics: ResourceMetrics | None = Field(
        default=None, description="Detailed resource usage and efficiency metrics"
    )
...
class UncertaintyGeometry(BaseModel):
    ...
    confidence: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Primary confidence estimate"
    )
    confidence_sources: list[str] = Field(
        default_factory=list,
        description="'prior_bias' | 'data_gap' | 'model_risk' | 'ambiguity' | 'time_sensitivity'",
    )
    model_risk: float = Field(
        ge=0.0, le=1.0, default=0.0, description="Uncertainty from model limitations"
    )
    data_gaps: list[str] = Field(default_factory=list, description="Named missing data")
    known_unknowns: list[str] = Field(
        default_factory=list, description="Things known to be unknown"
    )
    unknown_unknowns_suspected: list[str] = Field(
        default_factory=list, description="Potential gaps suspected but not named"
    )
    counterfactuals_considered: int = Field(
        default=0, ge=0, description="Number of alternatives evaluated"
    )
