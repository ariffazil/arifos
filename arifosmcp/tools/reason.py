...
def arif_mind_reason(
    ...
    if mode == "reflect":
        return Synthesis(
            **_ok("arif_mind_reason", {"query": query, "verdict": "PLAUSIBLE", "reflection": ""})
        )
    if mode == "forge":
        return Synthesis(
    ...
    if mode == "debate":
        return Synthesis(
            **_ok(
                "arif_mind_reason",
                {"query": query, "positions": ["pro", "con"], "resolution": "HOLD"},
            )
        )
    if mode == "socratic":
        return Synthesis(
            **_ok("arif_mind_reason", {"query": query, "questions": ["Why?", "What if not?"]})
        )
