...
def arif_paradox_status(
    ...
    cb1_msg = (
        ...
        if cb1_state == "TRIPPED"
        else "elevated confidence"
        if cb1_state == "WARNING"
        else "healthy epistemic humility"
    )
    results.append(
        {
            "breaker": "CB1",
    ...
    cb2_msg = (
        ...
        if cb2_state == "TRIPPED"
        else "marginally corroborated"
        if cb2_state == "WARNING"
        else "all witnesses above threshold"
    )
    results.append(
        {
            "breaker": "CB2",
    ...
    cb3_state = (
        "TRIPPED"
        if confidence > 0.99 and ev_level < 2
        else ("WARNING" if confidence > 0.95 and ev_level < 3 else "OK")
    )
    if cb3_state != "OK":
        unhealthy += 1
    cb3_msg = (
        "truth without cost"
        if cb3_state == "TRIPPED"
        else "confidence exceeds evidence"
        if cb3_state == "WARNING"
        else "confidence grounded in evidence"
    )
    results.append(
        {
            "breaker": "CB3",
    ...
    # CB4: Recursive Stack
    cb4_state = (
        "TRIPPED" if self_reference_depth > 5 else ("WARNING" if self_reference_depth > 3 else "OK")
    )
    if cb4_state != "OK":
        unhealthy += 1
    cb4_msg = (
        "infinite regress risk"
        if cb4_state == "TRIPPED"
        else "elevated recursion"
        if cb4_state == "WARNING"
        else "no recursive paradox"
    )
    results.append(
        {
            "breaker": "CB4",
    ...
    cb5_state = (
        "TRIPPED"
        if confidence_increase > 0.3 and not evidence_added
        else ("WARNING" if confidence_increase > 0.1 and not evidence_added else "OK")
    )
    if cb5_state != "OK":
        unhealthy += 1
    cb5_msg = (
        "belief hardening"
        if cb5_state == "TRIPPED"
        else "confidence inflation"
        if cb5_state == "WARNING"
        else "confidence growth matches evidence"
    )
    results.append(
        {
            "breaker": "CB5",
