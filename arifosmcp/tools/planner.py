...
async def arif_plan_and_simulate(
    intent: str, context: dict[str, Any], risk_tier: str = "medium", force_simulation: bool = True
) -> PlannerOutput:
    """
    ...
    if risk_tier in ["high"]:
        selected_plan = min(
            options, key=lambda x: x.simulation.risk_score if x.simulation else float("inf")
        )
    else:
    ...
    return PlannerOutput(
        ...
        advisory=f"Generated {len(options)} plan options. Selected {selected_plan.plan_id} based on risk."
        if selected_plan
