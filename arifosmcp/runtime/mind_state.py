...
class MINDState:
    ...
    def revise_step(self, step_number: int, new_content: str, reason: str = "") -> ReasoningStep:
        """Create a revised version of a failed step."""
        original = next((s for s in self.plan_steps if s.step_number == step_number), None)
    ...
    def rollback_to(self, checkpoint_idx: int = -1) -> bool:
        ...
        for key, value in snap.items():
            if key in ("plan_steps",):
                self.plan_steps = [ReasoningStep.from_dict(s) for s in value]
    ...
    def from_dict(cls, d: dict[str, Any]) -> MINDState:
        ...
        d = dict(d)
        d["plan_steps"] = [ReasoningStep.from_dict(s) for s in d.get("plan_steps", [])]
