        ...
        return json.dumps(
            {
                ...
                "required_fields": ["verdict", "floors_triggered", "confidence", "reasoning_class"],
            },
            indent=2,
        )
...
def register_schema_resources(mcp: FastMCP) -> None:
    ...
    def schema_tool(tool_id: str) -> str:
        ...
        return json.dumps(
            {"error": f"Tool {tool_id} not found", "available": list(registry.tool_packets.keys())}
        )
