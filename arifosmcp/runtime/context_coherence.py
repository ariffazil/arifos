"""
arifosmcp/runtime/context_coherence.py
═══════════════════════════════════════════════════════════════════════════════
EUREKA 2: Context Coherence Verifier (formerly "ZKPC Verifier")

Verifies a context claim against evidence across 7 dimensions.
Each dimension scores 0.0–1.0. PASS requires ALL dimensions >= 0.7.

This is a HEURISTIC scorer — NOT a cryptographic zero-knowledge proof.
It enforces F2 (TRUTH) and F7 (HUMILITY) by checking contextual coherence.

NOTE: Renamed from zkpc_verifier.py (2026-06-14, ZKPC-REALITY-ALIGN).
The old name implied cryptography where none exists.
...
def _score_dimension(dim: str, context_claim: dict, evidence: dict) -> float:
    """Score a single coherence dimension 0.0–1.0 based on available evidence."""
...
def verify_context_coherence(
    context_claim: dict,
    evidence: dict,
) -> tuple[bool, dict[str, float]]:
    """
    Verify a context claim against evidence across 7 coherence dimensions.
