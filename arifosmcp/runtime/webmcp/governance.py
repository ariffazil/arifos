...
class AgentDID(BaseModel):
    ...
    policy_level: str = Field(
        default="general", description="research|medical|financial|nuclear|general"
    )
...
class ActionRequest(BaseModel):
    ...
    action_type: str = Field(
        ..., description="tool_call|api_request|code_execution|data_modification"
    )
...
class GovernanceEvaluation(BaseModel):
    ...
    evaluation_id: str = Field(
        default_factory=lambda: f"gov-{hashlib.sha256(str(time.time()).encode()).hexdigest()[:16]}"
    )
    timestamp: float = Field(default_factory=time.time)
...
class GovernanceEngine:
    ...
    async def _scan_for_injection(self, request: ActionRequest) -> float:
        ...
        for pattern, _name, weight in guard.all_patterns:
            import re
