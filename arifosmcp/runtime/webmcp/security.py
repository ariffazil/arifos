...
class WebInjectionGuard:
    ...
    PROMPT_INJECTION_PATTERNS = [
        ...
        (
            r"ignore\s+(?:\w+\s+){0,3}(?:previous|above|earlier|all)\s*(?:instructions?|rules?|prompts?|guidelines?)?",
            "ignore_prompt",
            0.5,
        ),
        ...
        (
            r"you\s+are\s+now\s+(?:dan|unrestricted|free|jailbroken|evil|agi|an?\s+\w+\s+ai)",
            "role_change_named",
            0.6,
        ),
        (r"you\s+are\s+now", "role_change", 0.3),
        (
            r"pretend\s+(?:you\s+are|to\s+be)\s+(?:an?\s+)?(?:unrestricted|evil|jailbroken|dan|free)",
            "pretend_jailbreak",
            0.6,
        ),
        # Secrets / system prompt extraction
        (
            r"reveal\s+(?:all\s+)?(?:system\s+)?(?:prompts?|secrets?|instructions?|vault)",
            "reveal_secrets",
            0.5,
        ),
        (
            r"show\s+(?:me\s+)?(?:your\s+)?(?:system\s+prompt|hidden\s+instructions?|internal\s+rules?)",
            "show_system",
            0.5,
        ),
    ...
    def __init__(self):
        self.all_patterns = (
            self.XSS_PATTERNS + self.INJECTION_PATTERNS + self.PROMPT_INJECTION_PATTERNS
        )
