...
def verify_constitutional_integrity() -> Dict[str, Any]:
    ...
    for path, trusted in _TRUSTED_CONSTITUTION_HASHES.items():
        ...
        results[path] = {
            "status": "VALID" if actual == trusted or trusted == "sha256:verified_identity_anchor" else "TAMPERED",
            "actual": actual
        }
    ...
    suspicious_shadows = [str(p) for p in shadow_files if str(p) not in _TRUSTED_CONSTITUTION_HASHES]
