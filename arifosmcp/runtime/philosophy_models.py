"""
arifosmcp/runtime/philosophy_models.py — Typed Philosophy Models for MCP
...
class PhilosophyQuote(BaseModel):
    ...
    category: Literal[
        "wisdom", "discipline", "justice", "power", "paradox", "truth", "void", "seal"
    ] = Field(..., description="Governance category")
...
class PhilosophySelection(BaseModel):
    ...
    g_star: float | None = Field(
        None, ge=0.0, le=1.0, description="G★ score that influenced selection"
    )
...
class PhilosophyRegistryStats(BaseModel):
    ...
    diversity_score: float = Field(
        ..., ge=0.0, le=1.0, description="Shannon entropy diversity score (D)"
    )
...
class PhilosophyRegistry(BaseModel):
    ...
    quotes: list[PhilosophyQuote] = Field(
        ..., min_length=33, max_length=99, description="33-99 quotes (target: 99)"
    )
