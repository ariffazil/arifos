...
def build_evidence_plan(
    ...
    if (
        truth_classification.temporal_dependency
        or truth_classification.truth_class == TruthClass.TIME_SENSITIVE_FACT
    ):
...
def assess_ambiguity(
    ...
    if (
        ambiguity.detected
        and sense_input.intent.decision_proximity == DecisionProximity.DECISION_CRITICAL
    ):
...
def calculate_uncertainty(
    ...
    if ambiguity.detected:
        narrative_parts.append(
            f"ambiguity in {', '.join(a.value for a in ambiguity.ambiguity_type)}"
        )
    if conflict.detected:
...
def calculate_state_update(
    ...
    if ambiguity.detected:
        update.unknowns_delta.append(
            f"Ambiguity: {', '.join(a.value for a in ambiguity.ambiguity_type)}"
        )
...
def _map_a_forge_to_python_result(af_result, raw_input):
    ...
    uncertainty_score = {"low": 0.3, "medium": 0.5, "high": 0.7, "critical": 0.9}.get(
...
async def governed_sense_v2(
    ...
    if not _check_nested_depth(raw_input, max_depth=10):
        raise ValueError(
            "Constitutional Violation: Input depth exceeds safe limits (max_depth=10)."
        )
