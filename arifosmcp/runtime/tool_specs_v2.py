"""
arifosmcp/runtime/tool_specs_v2.py — arifOS MCP v2 Clean Sovereign Core

MCP-standard compliant tool specifications.
9 canonical tools, namespace versioning, visibility tiers.

Tool Naming: arifos.v2.{verb} format (namespace-versioned, SDK-safe)
Namespace: arifos.v2
Version: 2026.04.06-v2
Visibility: public (3) | internal (6)
...
V2_TOOL_COUNT = len(V2_TOOLS)

# Map old canonical names to v2 names (for migration)
V1_TO_V2_MAP: dict[str, str] = {
...
def get_v2_tool_spec(name: str) -> ToolSpecV2 | None:
    """Get v2 tool spec by name."""
...
def v2_tool_names() -> tuple[str, ...]:
    """Return all v2 tool names."""
    return tuple(t.name for t in V2_TOOLS)
