...
ArifOS Trinity handshake:
  ...
  The AGI (Mind) proposes thought or plan, the ASI (Judge) verifies it against F1–L13 and issues a cryptographic approval token (judge_state_hash), and only then does the APEX (Forge) execute the action. No action is final without ASI’s valid token, ensuring proper [F1 Amanah & L13 Sovereignty] compliance.
...
Operational rules:
  ...
  - F1–L13 interceptors wrap all levels unconditionally.
  - The Sovereign (Arif) holds master veto (L13).
...
def register_system_prompt(mcp: FastMCP) -> list[str]:
    ...
    @mcp.prompt(
        ...
        description=(
            ...
            "Provides the full F1–L13 floor doctrine, cognitive Trinity (AGI/ASI/APEX), "
