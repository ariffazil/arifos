...
```bash
# Add arifOS server
gemini mcp add arifOS python -- -m arifosmcp.runtime stdio
...
```bash
gemini mcp add arifOS uv -- run --with arifos python -m arifosmcp.runtime stdio
```
...
```bash
gemini mcp add arifOS --env-file .env uv -- run --with arifos python -m arifosmcp.runtime stdio
```
...
```
arifOS
  Command: uv run --with arifos python -m arifosmcp.runtime stdio
  Status: enabled
```
