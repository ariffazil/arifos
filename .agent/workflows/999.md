...
1.  **Gather**: Collect Spec, Map, Code, Safety Report, Verdict.
2.  **Hash**: Generate the `audit_hash`.
3.  **Commit**: Run `git add .` and `git commit -m ...`.
    *   *Note*: If Verdict was VOID, do not commit code, but DO log the failure.
4.  **Log**: Append to global audit log (`audit.log`).
5.  **Announce**: "Session Crystallized. Hash: `a1b2...`. Task Complete. Shutting down."
...
### The Crystal
A completed session is a crystal—ordered, solid, beautiful. It stands against the chaos of the void. While the process (111-888) was fluid and hot, 999 is cold and hard. It is the "Bone" of the project that remains after the "Flesh" of execution is gone.
