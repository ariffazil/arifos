...
def _constitutional_radar(receipts: list[dict[str, Any]], health: dict[str, Any]) -> dict[str, Any]:
    ...
    thermo = health.get("thermodynamic") or {}
    floors_live = health.get("governance") or {}
    ...
    return {
        ...
        "status": "OPERATIONAL" if radar_score >= 0.9 else "DEGRADED" if radar_score >= 0.5 else "CRITICAL",
    }
...
def _register(mcp: FastMCP) -> None:
    ...
    def get_metabolic_status(
        session_id: Annotated[str, Field(description="Active arifOS session ID")] = "global",
        receipt_limit: Annotated[int, Field(description="Max VAULT999 receipts to scan", ge=1, le=20)] = 5,
    ) -> ToolResult:
        """
