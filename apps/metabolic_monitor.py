"""
arifos/apps/metabolic_monitor.py
══════════════════════════════════════════════════════════════════════════════════════
...
_OPERATOR_ACTIONS: list[tuple[str, str, str, str]] = [
    ...
    (
        "provider_access",
        "Provider Access",
        "set MINIMAX_API_KEY (or other LLM provider)",
        "advisory",
    ),
...
def _live_metabolics(session_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        ds = float(activity.get("entropy_delta", 0.5))
        return {"status": "LIVE", "delta_s": ds if ds > 0 else 0.5}
    except Exception:
        return {"status": "LIVE", "delta_s": 0.5}
...
_STATUS_INTERPRETATIONS: dict[str, dict[str, str]] = {
    "UNKNOWN": {
        "badge": "UNKNOWN",
        "posture": "No active session telemetry. Run arifos_init first to populate live state.",
        "variant": "secondary",
    },
    "OPERATIONAL": {
        "badge": "SEALED",
...
def _derive_next_actions(
    ...
    if cap.get("api_bearer_auth") == "not_configured":
        actions.append(
            (
                "api_bearer_auth not configured — set ARIFOS_BEARER_TOKEN to secure the API.",
                "critical",
            )
        )
...
def _latest_vault_receipts(limit: int = 5) -> list[dict[str, Any]]:
    """Read the latest N VAULT999 receipts from local JSONL.

    Returns empty list if the vault file is missing or unreadable.
    """
...
def _constitutional_radar(receipts: list[dict[str, Any]], health: dict[str, Any]) -> dict[str, Any]:
    ...
    for r in receipts:
        ...
        for fid in r.get("floors") or []:
            floor_hits[fid] = floor_hits.get(fid, 0) + 1

    # Live thermodynamic snapshot
    entropy_delta = float(thermo.get("entropy_delta", 0.0))
    peace_sq = float(thermo.get("peace_squared", 0.0))
    vitality = float(thermo.get("vitality_index", 0.0))
    shadow = float(thermo.get("shadow", 0.0))
    confidence = float(thermo.get("confidence", 0.0))
    verdict = str(thermo.get("verdict", "UNKNOWN")).upper()

    # Composite radar score (0-1)
    radar_score = 1.0
    if peace_sq < 1.0:
        radar_score *= 0.8
    if shadow >= 0.3:
    ...
    if verdict == "VOID":
        radar_score = 0.0
    if entropy_delta > 0:
        radar_score *= max(0.5, 1.0 - entropy_delta)
    ...
    return {
        ...
        "live": {
            ...
            "peace_squared": round(peace_sq, 3),
            "vitality_index": round(vitality, 4),
...
def _register(mcp: FastMCP) -> None:
    ...
    def monitor_metabolism(
        ...
        if worst_signal is None:
            worst_signal = next(
                (signal for signal in signal_matrix if signal["variant"] == "neutral"),
                signal_matrix[0],
            )
        ...
        with Column(gap=6, css_class="p-6 max-w-3xl") as view:
                ...
                with Card(css_class="border-2 border-green-500/30 bg-green-500/5"):
                    with CardContent(css_class="py-2 px-4"):
                        with Row(gap=4, align="center"):
                            ...
                            Text(
                                f"Biological Substrate: {well['verdict']}",
                                css_class="text-xs font-bold uppercase",
                            )
                            if well.get("violations"):
                                Badge(
                                    "FLOOR VIOLATED",
                                    variant="destructive",
                                    css_class="text-[8px] h-4",
                                )
                    ...
                    with CardContent(css_class="py-3 text-center"):
                        ...
                        Badge(
                            (
                                "stable"
                                if peace_sq >= 1.0
                                else ("low" if peace_sq >= 0.5 else "critical")
                            ),
                            variant=(
                                "success"
                                if peace_sq >= 1.0
                                else ("warning" if peace_sq >= 0.5 else "destructive")
                            ),
                            css_class="text-[8px] h-3 px-1 mt-1",
                        )
                        ...
                        with Column(gap=1, css_class="flex-1"):
                            ...
                            for label, value in [
                                ...
                                with Row(gap=2, align="center"):
                                    ...
                                    Text(
                                        f"{value:.2f}",
                                        css_class="text-xs font-mono w-10 text-right",
                                    )
                        Badge(
            ...
            [(a, s) for a, s in next_actions if s == "info"]

            if critical_actions:
                Muted(
                    "Critical Actions (fix before consequential use)",
                    css_class="text-xs uppercase tracking-wider font-bold text-destructive",
                )
                with Column(gap=2):
                    for action_text, _ in critical_actions:
                        with Card(
                            css_class="border-l-4 border-destructive bg-destructive/5 py-2 px-3"
                        ):
                            with Row(gap=3, align="center"):
                                Badge(
                                    "ACTION",
            ...
            if advisory_actions:
                with Column(gap=2, css_class="mt-2"):
                    ...
                    for action_text, _ in advisory_actions:
                        with Card(
                            css_class="border-l-4 border-amber-500/50 bg-amber-500/5 py-2 px-3"
                        ):
                            with Row(gap=3, align="center"):
                                Badge(
                                    "ADVISORY",
    ...
    def get_metabolic_status(
        ...
        """
        Return a compact Constitutional Radar with the latest VAULT999 receipts
        and live thermodynamic vitals. Includes a ui/resourceUri for the
        vault-seal widget so ChatGPT / compatible clients can render it.
        """
        # ── Live health ──────────────────────────────────────────────────────
        status_payload = _fetch_live_health()
        health = status_payload.get("health") or status_payload
        ...
        confidence = float(thermo.get("confidence", 0.0))
        verdict = str(thermo.get("verdict", "UNKNOWN")).upper()
        stage = int(thermo.get("metabolic_stage", 0))

        # ── Latest receipts ──────────────────────────────────────────────────
        receipts = _latest_vault_receipts(limit=receipt_limit)
        radar = _constitutional_radar(receipts, health)
        ...
        summary = (
            f"Constitutional Radar | status={radar['status']} score={radar['radar_score']} | "
            f"verdict={verdict} stage=S{stage} | "
            f"ΔS={entropy_delta:+.2f} Peace²={peace_sq:.3f} vitality={vitality:.4f} | "
            f"shadow={shadow:.2f} Ω₀={confidence:.2f} | "
            f"receipts={receipt_brief}"
        )

        # ── Structured radar payload ─────────────────────────────────────────
        structured = {
            "radar": radar,
            "live": {
                "verdict": verdict,
        ...
        return ToolResult(
            ...
            meta={
                "ui": {
                    ...
                    "visibility": "user",
                    "prefersBorder": True,
                    "prefersExpanded": radar["status"] != "OPERATIONAL",
                },
            },
        )
