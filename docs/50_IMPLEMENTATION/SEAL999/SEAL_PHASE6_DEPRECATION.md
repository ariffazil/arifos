...
1. FastMCP project config now targets AAA surface
   - `fastmcp.json` source path → `arifosmcp.runtime/server.py`
   - `prod.fastmcp.json` source path → `arifosmcp.runtime/server.py`
   - `dev.fastmcp.json` source path → `arifosmcp.runtime/server.py`

2. Legacy package reduced to compatibility facade
   - `arifosmcp.transport/__init__.py` exports only `mcp` eagerly.
