...
2. **Phase 1 (done):** established new package boundary `arifosmcp.runtime` without breaking current `arifosmcp.transport`.
...
5. **Phase 4 (done):** set `arifosmcp.runtime` as default external runtime + law embedding bridge.
...
- Added `arifosmcp.runtime/` package with canonical 13-tool router.
- Added `arifosmcp.runtime/governance.py` with internal `AXIOMS_333`:
  - `A1_TRUTH_COST`
  - `A2_SCAR_WEIGHT`
  - `A3_ENTROPY_WORK`
- Added static contract tests in `tests/test_aaa_mcp_contract.py`.
- Added `arifosmcp.runtime/contracts.py` for session continuity and input guard checks.
- Added `tests/test_aaa_phase3_flow.py` to verify phase-3 guard wiring and 000->999 tool presence.
- Added `arifosmcp.runtime/fastmcp_ext/` boundary modules to keep FastMCP SDK concerns outside `core/`.
