...
class PolicyCondition:
    ...
    def evaluate(self, organ_state: dict) -> bool:
        ...
        if self.operator == "==": return actual == self.value
        if self.operator == "!=": return actual != self.value
        if self.operator == "in": return actual in (self.value or [])
        return False
...
STARTER_POLICIES = [
    ScenarioPolicy(
        ...
        any_of=[
            ...
            PolicyCondition(organ="WELL", metric="fatigue", operator="in", value=["DEGRADED", "CRITICAL"]),
        ],
        action="HOLD",
...
def evaluate_event(event: dict, organ_state: dict, policies: list[ScenarioPolicy] = None) -> list[dict]:
