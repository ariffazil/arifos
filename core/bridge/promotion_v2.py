...
class PromotionBridge:
    ...
    def classify_for_promotion(
    ...
    ) -> tuple[bool, str, PromotionOutcome | None]:
        """
        ...
        if memory.governance.confidence < 0.85:
            return (
                False,
                f"Confidence {memory.governance.confidence} < 0.85",
                PromotionOutcome.REJECTED_NON_CONSEQUENTIAL,
            )
        ...
        if memory.governance.sensitivity.value == "high":
            if not memory.source.origin.value == "user":
                return (
                    False,
                    "High-risk requires human assertion",
                    PromotionOutcome.HELD_PENDING_HUMAN_CONFIRMATION,
                )
