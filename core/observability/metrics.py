...
class LiveMetrics:
    ...
    def record_step(self, session_id: str, latency: float, energy: float, success: bool, error: Optional[str] = None):
...
class SystemAuditor:
    ...
    def check_5_organ_membrane(cls, arifos_root: Path) -> tuple[bool, list[str]]:
        ...
        required_organs = [
            ("init_session", "Ψ"), ("agi_cognition", "Δ"),
            ("asi_empathy", "Ω"), ("apex_verdict", "Ψ"), ("vault_seal", "F1"),
        ]
        for organ, symbol in required_organs:
            if f'@mcp.tool(name="{organ}"' not in content:
                violations.append(f"MISSING ORGAN: {organ} ({symbol})")
                
        legacy_patterns = ['"reason"', '"validate"', '"align"', '"forge"', '"audit"', "9 A-CLIP skills"]
            ...
            try:
                ...
                for lib in transport_libs:
                    if f"import {lib}" in source or f"from {lib}" in source:
                        violations.append(f"VIOLATION: {py_file.name} imports '{lib}' (core/ must be PURE LOGIC)")
            except Exception:
                pass
        return len(violations) == 0, violations
