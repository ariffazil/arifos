...
from arifosmcp.core.shared.types import DeltaBundle, GPV, QueryType
...
def gpv_f2_threshold(gpv: GPV) -> float:
    """Adaptive F2 threshold for a given GPV."""
    thresholds = {
...
def gpv_f4_skip(gpv: GPV) -> bool:
    """Check if F4 expansion should be skipped for this GPV."""
    return (
        gpv.query_type
        in (
...
class ATLAS:
    ...
    def audit_humility(self, confidence: float) -> bool:
        """
        F7 Humility Audit: Validate confidence against \u03a9\u2080 band.

        Target: \u03a9\u2080 \u2208 [0.03, 0.05]. Overconfidence/Underconfidence triggers VOID.
        """
        # Lower bound: 0.03 (Avoid pseudo-certainty)
        # Upper bound: 0.05 (Avoid epistemic nihilism)
        return 0.03 <= confidence <= 0.05

    def detect_paradox(self, reasoning: str) -> list[str]:
        """
        F7 Paradox Detection: Scan for direct and circular contradictions.
        """
        scars = []
        text_lower = reasoning.lower()

        # Direct contradiction pattern (A and NOT A)
        # Simple heuristic for now; more complex logic happens in the LLM engine
        if re.search(r"\b(is|are)\b.*\b(is not|are not|isn't|aren't)\b", text_lower):
            scars.append("potential_direct_contradiction")

        # Circular logic detection
        if re.search(r"(\b\w+\b).*\sbecause\s.*(\b\1\b)", text_lower):
            scars.append("potential_circular_reasoning")
    ...
    def assemble_delta_bundle(
        ...
        """
        Assemble the canonical Delta Bundle for Stage 333.
        """
        scars = self.detect_paradox(reasoning)

        # F7 Humility Law: Clamp confidence to \u03a9\u2080 band if outside
        if not self.audit_humility(confidence):
            logger.warning(f"Humility violation: confidence {confidence} outside [0.03, 0.05]. Clamping.")
            confidence = max(0.03, min(0.05, confidence))

        return DeltaBundle(
            facts=facts,
            reasoning_tree={"raw_text": reasoning},
            confidence_omega_0=confidence,
            scars=scars,
            entropy_delta_s=entropy,
            floor_scores={
                "F2": gpv.tau,
                "F4": 1.0 - max(0, entropy),
                "F7": 1.0 if not scars else 0.5,
                "F13": (gpv.tau > 0.7 or gpv.rho > 0.5),
            },
            gpv=gpv,
        )
