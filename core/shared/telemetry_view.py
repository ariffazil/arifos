...
def build_human_view(
    ...
    raw_floors: dict[str, Any],
    raw_witness: dict[str, Any],
    verdict: str,
    epoch_label: str = "EPOCH-NOW",
) -> dict[str, Any]:
    """
    ...
    return {
        ...
        "raw": {
            ...
            "witness": raw_witness,
            "verdict": {"state": verdict, "authority": "888_JUDGE"},
        },
        "human": {
            "summary": {
        ...
        "display_rules": {
            "humility_band": {
                ...
                "current": f"{combined.get('omega_0', 0) * 100:.0f}% — "
                + (
                    "in healthy range"
                    if 0.03 <= combined.get("omega_0", 0) <= 0.05
                    else "out of bounds"
                ),
            },
            "color_coding": {
