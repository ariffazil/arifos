...
async def forge(
    ...
    if query_type_value in [QueryType.TEST, QueryType.CONVERSATIONAL]:
        ...
        asi_out = await asi(
            action="full", 
            agi_tensor=agi_out.tensor if hasattr(agi_out, "tensor") else None,
            delta_bundle=agi_out.delta_bundle if hasattr(agi_out, "delta_bundle") else None,
            session_id=token.session_id, 
            query=query
        )
        ...
        apex_out = await apex(
            ...
            session_id=token.session_id,
            intent=query,
            omega_bundle=asi_out.omega_bundle if hasattr(asi_out, "omega_bundle") else None,
            floor_scores=asi_out.floor_scores if hasattr(asi_out, "floor_scores") else {}
        )
