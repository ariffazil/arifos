...
class EpistemicAnchorClient:
    ...
    async def anchor_seal(self, seal_hash: str, metadata: dict) -> BlockchainAnchor:
        ...
        anchor = BlockchainAnchor(
            seal_hash=seal_hash, timestamp=datetime.utcnow(), cloud_replicas=[]
        )
    ...
    async def _anchor_to_polygon(self, seal_hash: str, metadata: dict) -> str:
        ...
        if not private_key:
            raise RuntimeError("POLYGON_PRIVATE_KEY not set — cannot anchor to Polygon")
    ...
    async def _store_gcs(self, seal_hash: str, metadata: dict) -> str:
        ...
        if not key_json:
            raise RuntimeError("GCP_SERVICE_ACCOUNT_KEY not set — cannot store to GCS")
    ...
    async def _store_b2(self, seal_hash: str, metadata: dict) -> str:
        ...
        if not app_key_id or not app_key:
            raise RuntimeError("B2_APPLICATION_KEY_ID or B2_APPLICATION_KEY not set")

        auth_str = base64.b64encode(f"{app_key_id}:{app_key}".encode()).decode()
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(
                ...
                if resp.status != 200:
                    raise RuntimeError(f"B2 get_upload_url failed: {resp.status}")
