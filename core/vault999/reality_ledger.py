...
def append_to_ledger(entry_data: dict) -> str:
    ...
    required_keys = ["trace_id", "principal", "intent", "claims", "evidence", 
                     "prediction", "plan", "approval", "execution", "outcome", "delta", "seals"]
...
def replay_trace(trace_id: str):
    ...
    for entry in entries:
        ...
        print(f"  ▶ INTENT:    {intent.get('description', 'N/A')} (Risk: {intent.get('risk_class', 'N/A')})")
        plan = entry.get('plan', {})
        print(f"  ▶ PLAN:      {plan.get('selected_plan', 'N/A')}")
        approval = entry.get('approval', {})
        print(f"  ▶ APPROVAL:  {approval.get('judge_verdict', 'N/A')}")
        execution = entry.get('execution', {})
        print(f"  ▶ EXECUTION: {execution.get('adapter', 'N/A')} -> {execution.get('target', 'N/A')}")
        outcome = entry.get('outcome', {})
        print(f"  ▶ OUTCOME:   {outcome.get('observed_result', 'N/A')} (Success: {outcome.get('success', False)})")
        print("-" * 50)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--limit", type=int, default=10, help="Number of recent entries to show if no trace_id")
    ...
    if args.trace_id:
        ...
        for e in entries:
            print(f"{e.get('timestamp')} | Trace: {e.get('trace_id')} | Intent: {e.get('intent', {}).get('description', 'N/A')}")
