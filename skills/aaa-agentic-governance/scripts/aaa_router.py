...
def route_organs(text: str) -> list[Route]:
    ...
    if not matches:
        matches.append(
            Route("AAA", 0.58, "default control-plane abstraction for ambiguous federation task", 0)
        )
...
def infer_attestation(text: str) -> Attestation:
    ...
    lowered = text.lower()
    if re.search(
        r"\b(live|health|verified|test output|tool returned|observed|log shows)\b", lowered
    ):
    ...
    if re.search(r"\b(assume|probably|hypothesis|maybe|infer|likely)\b", lowered):
        return Attestation(
            "HYPOTHESIS", ["must carry falsifier; not presentable as verified state"]
        )
    ...
    return Attestation(
        "INFERRED", ["confirm with repo/runtime evidence before high-consequence action"]
    )
...
def hold_conditions_for(tier: int, routes: Iterable[Route]) -> list[str]:
    ...
    if tier == 3:
        conditions.append(
            "do not execute irreversible or constitutional mutations; 888_HOLD until F13 ack"
        )
    ...
    if "A-FORGE" in organ_names:
        conditions.append(
            "A-FORGE execution requires approved plan and judge/approval gate for high-risk work"
        )
    if "GEOX" in organ_names:
        conditions.append(
            "GEOX must provide evidence, uncertainty, and claim limits; no drilling decision alone"
        )
    if "WEALTH" in organ_names:
        conditions.append(
            "WEALTH must expose assumptions/downside risk; no capital allocation alone"
        )
...
def route(
    text: str, explicit_organ: str | None = None, max_recursion_depth: int = DEFAULT_RECURSION_DEPTH
) -> RoutingResult:
    if explicit_organ:
        ...
        routes = [
            Route(
                explicit_organ, 1.0, "operator-forced explicit organ (F10 cardinality respected)", 0
            )
        ]
    else:
    ...
    abduction = Abduction(
        ...
        validation_step="inspect source-of-truth before mutation"
        if tier >= 1
        else "state evidence class and proceed read-only",
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Route an AAA governed intelligence request (v3, 8-cardinality)."
    )
    ...
    result = route(
        text, explicit_organ=args.explicit_organ, max_recursion_depth=args.max_recursion_depth
    )
