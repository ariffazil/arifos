...
def _compute_risk(baseline: str, task: str, consent_reliability: float = 1.0,
                   strain_debt: float = 0.0) -> str:
    """
...
def _compute_mode(consent_reliability: float, task_readiness: str,
                  convergence_alert: dict | None) -> str:
    """
...
def _compute_failure_flags(metrics: dict, hrv_velocity: float,
                            consent_reliability: float) -> list[str]:
...
def readiness_check(task_load: float = 0.5,
                    override_state: dict | None = None) -> dict[str, Any]:
    """
    ...
    try:
        ...
        task_readiness = _infer_task_readiness(
            consent_reliability * 100, violations, task_load
        )

        # ── Coupled risk ───────────────────────────────────────────────────
        coupled_risk = _compute_risk(baseline, task_readiness,
                                     consent_reliability, strain_debt)

        # ── Recommended mode ──────────────────────────────────────────────
        recommended_mode = _compute_mode(consent_reliability, task_readiness,
                                          convergence_alert)

        # ── Failure flags ──────────────────────────────────────────────────
        failure_flags = _compute_failure_flags(metrics, hrv_velocity,
                                               consent_reliability)
